...
OPERATE_RET tkl_disp_flush(TKL_DISP_DEVICE_S *display_device)
{
    if (fush_buf.buffer != NULL && fush_buf.disp_rect.height > 0 && fush_buf.disp_rect.width > 0) {
        disp_driver_flush(fush_buf.disp_rect.x, fush_buf.disp_rect.y,
                          fush_buf.disp_rect.x + fush_buf.disp_rect.width - 1,
                          fush_buf.disp_rect.y + fush_buf.disp_rect.height - 1, (uint8_t *)fush_buf.buffer);
    } else {
