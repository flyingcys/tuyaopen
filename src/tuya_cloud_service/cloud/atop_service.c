...
#define ACTIVATE_POST_BUFFER_LEN (255)
    ...
    /* Requires params */
    offset = snprintf(buffer, prealloc_size,
                      "{\"token\":\"%s\",\"softVer\":\"%s\",\"productKey\":\"%"
                      "s\",\"protocolVer\":\"%s\",\"baselineVer\":\"%s\"
                      request->token,
                      request->sw_ver, request->product_key, request->pv, request->bv);

    /* option params */
    offset += snprintf(buffer + offset, prealloc_size - offset, ",\"options\": \"%s\"", "{\"otaChannel\":0, ");
    ...
    offset += snprintf(buffer + offset, prealloc_size - offset,
                       ",\"cadVer\":\"%s\",\"cdVer\":\"%s\",\"t\":%" PRIu32 "}", CAD_VER, CD_VER, timestamp);
