...
#if LV_USE_OBSERVER && LV_USE_SLIDER && LV_USE_LABEL && LV_USE_ROLLER && LV_USE_DROPDOWN && LV_FONT_MONTSERRAT_30 &&   \
    LV_BUILD_EXAMPLES
...
static void time_observer_cb(lv_observer_t *observer, lv_subject_t *subject)
{
    ...
    if (format == TIME_FORMAT_24) {
        ...
        lv_label_set_text_fmt(label, "%" LV_PRId32 ":%02" LV_PRId32 " %s", hour + 1, minute,
                              am_pm == TIME_AM ? "am" : "pm");
    }
}
