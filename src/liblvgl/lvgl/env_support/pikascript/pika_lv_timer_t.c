...
{
    PikaObj *eventHandleObj = pks_eventListener_getEventHandleObj(g_pika_lv_timer_event_listener, (uint32_t)timer);
...
void pika_lvgl_lv_timer_t_set_cb(PikaObj *self, Arg *cb)
{
    ...
    pks_eventListener_registerEvent(g_pika_lv_timer_event_listener, (uint32_t)lv_timer, self);
}
