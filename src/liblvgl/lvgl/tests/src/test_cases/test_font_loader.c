...
static int compare_fonts(lv_font_t *f1, lv_font_t *f2)
{
    ...
    for (int i = 0; i < dsc1->cmap_num; ++i) {
        ...
        if (cmaps1->unicode_list != NULL && cmaps2->unicode_list != NULL) {
            ...
            TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE((uint8_t *)cmaps1->unicode_list, (uint8_t *)cmaps2->unicode_list,
    ...
    if (dsc1->kern_classes == 1 && dsc2->kern_classes == 1) {
        ...
        if (kern1 != NULL && kern2 != NULL) {
            ...
            TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE((uint8_t *)kern1->left_class_mapping,
                                                  (uint8_t *)kern2->left_class_mapping, kern1->left_class_cnt,
                                                  "left_class_mapping");

            TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE((uint8_t *)kern1->right_class_mapping,
                                                  (uint8_t *)kern2->right_class_mapping, kern1->right_class_cnt,
                                                  "right_class_mapping");

            TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE((uint8_t *)kern1->class_pair_values,
        ...
        if (kern1 != NULL && kern2 != NULL) {
            ...
            TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE((uint8_t *)kern1->values, (uint8_t *)kern2->values, kern1->pair_cnt,
                                                  "glyph_values");
        }
    }
    ...
    for (int i = 0; i < total_glyphs; ++i) {
        if (i < total_glyphs - 1) {
            ...
            if (size1 > 0) {
                TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE(dsc1->glyph_bitmap + glyph_dsc1[i].bitmap_index,
