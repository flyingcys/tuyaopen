...
#define N 8
    ...
    static lv_palette_t colors[] = {LV_PALETTE_BLUE,  LV_PALETTE_PURPLE,    LV_PALETTE_GREEN, LV_PALETTE_PINK,
                                    LV_PALETTE_AMBER, LV_PALETTE_BLUE_GREY, LV_PALETTE_INDIGO};
    ...
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < N; ++j) {
            obj0s[i][j] =
                obj_create(obj0, lv_palette_main(colors[(i * M + j) % ((sizeof colors) / (sizeof(lv_palette_t)))]));
        }
    }
