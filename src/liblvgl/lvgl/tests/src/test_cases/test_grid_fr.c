...
{
    static int32_t col_dsc[] = {3 * 9,         LV_GRID_FR(1), LV_GRID_FR(2),
                                LV_GRID_FR(3), LV_GRID_FR(4), LV_GRID_TEMPLATE_LAST};
    static int32_t row_dsc[] = {3 * 3,         LV_GRID_FR(1), LV_GRID_FR(1),
    ...
    for (int col = 0; col < 5; col++) {
        for (int row = 0; row < 5; row++) {
            ...
            lv_obj_set_style_bg_color(child, lv_palette_main((row + col) % 2 ? LV_PALETTE_GREEN : LV_PALETTE_BLUE),
                                      LV_PART_MAIN);
