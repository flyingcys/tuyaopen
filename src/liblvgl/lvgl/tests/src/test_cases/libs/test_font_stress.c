...
static const char *font_name_arr[] = {"./src/test_files/fonts/noto/NotoSansSC-Regular.ttf",
                                      "../src/libs/freetype/arial.ttf",
                                      "../demos/multilang/assets/fonts/Montserrat-Bold.ttf", "UNKNOWN_FONT_NAME"};
...
static lv_obj_t *label_create(const char *font_name, lv_obj_t *par, int size, int x, int y)
{
    ...
    uint8_t str[128];
    lv_random_utf8_chars(str, sizeof(str), LV_RND_UNICODE_ALPHANUM_AND_CJK_TABLE,
                         LV_RND_UNICODE_ALPHANUM_AND_CJK_TABLE_LEN, MAX_LABEL_CONTENT_LEN);
...
static void update_cb(void)
{
    ...
    if (label) {
        ...
        label = label_create(pathname, ctx->par, (int)font_size, (int)label_x, (int)label_y);
