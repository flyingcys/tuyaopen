...
static const char *compressions[] = {"UNCOMPRESSED",
...
void test_image_formats(void)
{
    for (unsigned align = 0; align <= 1; align++) {
        ...
        for (unsigned mode = 0; mode <= 3; mode++) {
            ...
            if (rotate)
                continue; /* Transform relies on LV_BIN_DECODER_RAM_LOAD to be enabled */
#endif
            ...
            for (unsigned i = 0; i < sizeof(compressions) / sizeof(compressions[0]); i++) {
                ...
                lv_snprintf(reference, sizeof(reference), "draw/bin_image_stride%d_%s_%s.png", stride, compressions[i],
                            modes[mode]);
                TEST_ASSERT_EQUAL_SCREENSHOT(reference);
                lv_obj_clean(lv_screen_active());

                c_array_image_create(rotate, recolor, align, i);
                lv_snprintf(reference, sizeof(reference), "draw/c_array_image_stride%d_%s_%s.png", stride,
                            compressions[i], modes[mode]);
                TEST_ASSERT_EQUAL_SCREENSHOT(reference);
                lv_obj_clean(lv_screen_active());
            }
        }
