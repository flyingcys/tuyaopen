...
static void lv_windows_pointer_driver_read_callback(lv_indev_t *indev, lv_indev_data_t *data);

static void lv_windows_release_pointer_device_event_callback(lv_event_t *e);

static void lv_windows_keypad_driver_read_callback(lv_indev_t *indev, lv_indev_data_t *data);

static void lv_windows_release_keypad_device_event_callback(lv_event_t *e);

static void lv_windows_encoder_driver_read_callback(lv_indev_t *indev, lv_indev_data_t *data);
...
lv_indev_t *lv_windows_acquire_pointer_indev(lv_display_t *display)
{
    ...
    if (!context->pointer.indev) {
        ...
        if (context->pointer.indev) {
            lv_indev_set_type(context->pointer.indev, LV_INDEV_TYPE_POINTER);
            lv_indev_set_read_cb(context->pointer.indev, lv_windows_pointer_driver_read_callback);
            lv_indev_set_display(context->pointer.indev, context->display_device_object);
            lv_indev_add_event_cb(context->pointer.indev, lv_windows_release_pointer_device_event_callback,
                                  LV_EVENT_DELETE, context->pointer.indev);
            lv_indev_set_group(context->pointer.indev, lv_group_get_default());
        }
    }
...
lv_indev_t *lv_windows_acquire_keypad_indev(lv_display_t *display)
{
    ...
    if (!context->keypad.indev) {
        lv_ll_init(&context->keypad.queue, sizeof(lv_windows_keypad_queue_item_t));
        context->keypad.utf16_high_surrogate = 0;
        context->keypad.utf16_low_surrogate = 0;

        context->keypad.indev = lv_indev_create();
        if (context->keypad.indev) {
            lv_indev_set_type(context->keypad.indev, LV_INDEV_TYPE_KEYPAD);
            lv_indev_set_read_cb(context->keypad.indev, lv_windows_keypad_driver_read_callback);
            lv_indev_set_display(context->keypad.indev, context->display_device_object);
            lv_indev_add_event_cb(context->keypad.indev, lv_windows_release_keypad_device_event_callback,
                                  LV_EVENT_DELETE, context->keypad.indev);
...
lv_indev_t *lv_windows_acquire_encoder_indev(lv_display_t *display)
{
    ...
    if (!context->encoder.indev) {
        ...
        if (context->encoder.indev) {
            lv_indev_set_type(context->encoder.indev, LV_INDEV_TYPE_ENCODER);
            lv_indev_set_read_cb(context->encoder.indev, lv_windows_encoder_driver_read_callback);
            lv_indev_set_display(context->encoder.indev, context->display_device_object);
            lv_indev_add_event_cb(context->encoder.indev, lv_windows_release_encoder_device_event_callback,
                                  LV_EVENT_DELETE, context->encoder.indev);
            lv_indev_set_group(context->encoder.indev, lv_group_get_default());
        }
    }
...
static void lv_windows_pointer_driver_read_callback(lv_indev_t *indev, lv_indev_data_t *data)
{
    lv_windows_window_context_t *context = lv_windows_get_window_context(lv_windows_get_indev_window_handle(indev));
    if (!context) {
        return;
    }
...
static BOOL lv_windows_get_touch_input_info(HTOUCHINPUT touch_input_handle, UINT input_count, PTOUCHINPUT inputs,
                                            int item_size)
{
    ...
    function_type function = (function_type)(GetProcAddress(module_handle, "GetTouchInputInfo"));
    if (!function) {
        return FALSE;
    }
...
static BOOL lv_windows_close_touch_input_handle(HTOUCHINPUT touch_input_handle)
{
    HMODULE module_handle = GetModuleHandleW(L"user32.dll");
    if (!module_handle) {
        return FALSE;
    ...
    function_type function = (function_type)(GetProcAddress(module_handle, "CloseTouchInputHandle"));
    if (!function) {
        return FALSE;
    }
                                                      ...
                                                      LRESULT *plResult)
{
    ...
    case WM_MOUSEMOVE: {
        ...
        if (context) {
            int32_t hor_res = lv_display_get_horizontal_resolution(context->display_device_object);
            int32_t ver_res = lv_display_get_vertical_resolution(context->display_device_object);

            context->pointer.point.x = lv_windows_zoom_to_logical(GET_X_LPARAM(lParam), context->zoom_level);
            context->pointer.point.y = lv_windows_zoom_to_logical(GET_Y_LPARAM(lParam), context->zoom_level);
            if (context->simulator_mode) {
                context->pointer.point.x = lv_windows_dpi_to_logical(context->pointer.point.x, context->window_dpi);
                context->pointer.point.y = lv_windows_dpi_to_logical(context->pointer.point.y, context->window_dpi);
            }
    ...
    case WM_LBUTTONUP: {
        lv_windows_window_context_t *context = (lv_windows_window_context_t *)(lv_windows_get_window_context(hWnd));
        if (context) {
            context->pointer.state = (uMsg == WM_LBUTTONDOWN ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED);
        }
    ...
    case WM_TOUCH: {
        ...
        if (context) {
            ...
            if (inputs) {
                if (lv_windows_get_touch_input_info(touch_input_handle, input_count, inputs, sizeof(TOUCHINPUT))) {
                    for (UINT i = 0; i < input_count; ++i) {
                        ...
                        context->pointer.point.x = lv_windows_zoom_to_logical(Point.x, context->zoom_level);
                        context->pointer.point.y = lv_windows_zoom_to_logical(Point.y, context->zoom_level);
                        if (context->simulator_mode) {
                            context->pointer.point.x =
                                lv_windows_dpi_to_logical(context->pointer.point.x, context->window_dpi);
                            context->pointer.point.y =
                                lv_windows_dpi_to_logical(context->pointer.point.y, context->window_dpi);
                        }

                        DWORD MousePressedMask = TOUCHEVENTF_MOVE | TOUCHEVENTF_DOWN;

                        context->pointer.state =
                            (inputs[i].dwFlags & MousePressedMask ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED);
                    }
                }
...
static void lv_windows_keypad_driver_read_callback(lv_indev_t *indev, lv_indev_data_t *data)
{
    ...
    lv_windows_keypad_queue_item_t *current =
        (lv_windows_keypad_queue_item_t *)(lv_ll_get_head(&context->keypad.queue));
    if (current) {
...
static void lv_windows_push_key_to_keyboard_queue(lv_windows_window_context_t *context, uint32_t key,
                                                  lv_indev_state_t state)
{
    lv_windows_keypad_queue_item_t *current =
        (lv_windows_keypad_queue_item_t *)(lv_ll_ins_tail(&context->keypad.queue));
    if (current) {
        current->key = key;
        current->state = state;
    }
...
static HIMC lv_windows_imm_get_context(HWND window_handle)
{
    ...
    function_type function = (function_type)(GetProcAddress(module_handle, "ImmGetContext"));
    if (!function) {
        return FALSE;
    }
...
static BOOL lv_windows_imm_release_context(HWND window_handle, HIMC imm_context_handle)
{
    HMODULE module_handle = GetModuleHandleW(L"imm32.dll");
    if (!module_handle) {
        return FALSE;
    ...
    function_type function = (function_type)(GetProcAddress(module_handle, "ImmReleaseContext"));
    if (!function) {
        return FALSE;
    }
...
static HIMC lv_windows_imm_associate_context(HWND window_handle, HIMC imm_context_handle)
{
    HMODULE module_handle = GetModuleHandleW(L"imm32.dll");
    if (!module_handle) {
        return FALSE;
    ...
    function_type function = (function_type)(GetProcAddress(module_handle, "ImmAssociateContext"));
    if (!function) {
        return FALSE;
    }
...
static BOOL lv_windows_imm_set_composition_window(HIMC imm_context_handle, LPCOMPOSITIONFORM composition_form)
{
    HMODULE module_handle = GetModuleHandleW(L"imm32.dll");
    if (!module_handle) {
        return FALSE;
    ...
    function_type function = (function_type)(GetProcAddress(module_handle, "ImmSetCompositionWindow"));
    if (!function) {
        return FALSE;
    }
                                                     ...
                                                     LRESULT *plResult)
{
        ...
        if (context) {
            ...
            if (!skip_translation) {
                lv_windows_push_key_to_keyboard_queue(
                    context, translated_key, ((uMsg == WM_KEYUP) ? LV_INDEV_STATE_RELEASED : LV_INDEV_STATE_PRESSED));
            }
        }
    ...
    case WM_CHAR: {
        ...
        if (context) {
            ...
            if (raw_code_point >= 0x20 && raw_code_point != 0x7F) {
                ...
                if (context->keypad.utf16_high_surrogate && context->keypad.utf16_low_surrogate) {
                    uint16_t high_surrogate = context->keypad.utf16_high_surrogate;
                    uint16_t low_surrogate = context->keypad.utf16_low_surrogate;
                ...
                uint32_t lvgl_code_point = lv_text_unicode_to_encoded(code_point);

                lv_windows_push_key_to_keyboard_queue(context, lvgl_code_point, LV_INDEV_STATE_PRESSED);
                lv_windows_push_key_to_keyboard_queue(context, lvgl_code_point, LV_INDEV_STATE_RELEASED);
            }
        }
    ...
    case WM_IME_SETCONTEXT: {
        if (wParam == TRUE) {
            ...
            if (imm_context_handle) {
                lv_windows_imm_associate_context(hWnd, imm_context_handle);
                lv_windows_imm_release_context(hWnd, imm_context_handle);
            }
        }
    ...
    case WM_IME_STARTCOMPOSITION: {
        ...
        if (imm_context_handle) {
            ...
            if (focused_object) {
                const lv_obj_class_t *object_class = lv_obj_get_class(focused_object);
            ...
            if (textarea_object) {
                ...
                composition_form.ptCurrentPos.x = label_object->coords.x1 + textarea->cursor.area.x1;
                composition_form.ptCurrentPos.y = label_object->coords.y1 + textarea->cursor.area.y1;
            }

            lv_windows_imm_set_composition_window(imm_context_handle, &composition_form);
            lv_windows_imm_release_context(hWnd, imm_context_handle);
        }
                                                      ...
                                                      LRESULT *plResult)
{
    ...
    case WM_MBUTTONUP: {
        lv_windows_window_context_t *context = (lv_windows_window_context_t *)(lv_windows_get_window_context(hWnd));
        if (context) {
            context->encoder.state = (uMsg == WM_MBUTTONDOWN ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED);
        }
    ...
    case WM_MOUSEWHEEL: {
        lv_windows_window_context_t *context = (lv_windows_window_context_t *)(lv_windows_get_window_context(hWnd));
        if (context) {
            context->encoder.enc_diff = -(GET_WHEEL_DELTA_WPARAM(wParam) / WHEEL_DELTA);
        }
