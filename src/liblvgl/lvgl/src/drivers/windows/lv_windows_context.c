...
static bool lv_windows_window_message_callback_nolock(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                      LRESULT *plResult);

static LRESULT CALLBACK lv_windows_window_message_callback(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
void lv_windows_platform_init(void)
{
    ...
    lv_timer_create(lv_windows_check_display_existence_timer_callback, 200, NULL);
...
{
    return (lv_windows_window_context_t *)(GetPropW(window_handle, L"LVGL.Window.Context"));
}
...
static void lv_windows_delay_callback(uint32_t ms)
{
    HANDLE timer_handle = CreateWaitableTimerExW(
        NULL, NULL, CREATE_WAITABLE_TIMER_MANUAL_RESET | CREATE_WAITABLE_TIMER_HIGH_RESOLUTION, TIMER_ALL_ACCESS);
    if (timer_handle) {
...
static HDC lv_windows_create_frame_buffer(HWND window_handle, LONG width, LONG height, UINT32 **pixel_buffer,
...
#error[lv_windows] Unsupported LV_COLOR_DEPTH.
        ...
        bitmap_info.bmiHeader.biBitCount = lv_color_format_get_bpp(LV_COLOR_FORMAT_NATIVE);
#if (LV_COLOR_DEPTH == 32) || (LV_COLOR_DEPTH == 24)
...
#error[lv_windows] Unsupported LV_COLOR_DEPTH.
        ...
        HBITMAP hBitmap = CreateDIBSection(frame_buffer_dc_handle, (PBITMAPINFO)(&bitmap_info), DIB_RGB_COLORS,
                                           (void **)pixel_buffer, NULL, 0);
        if (hBitmap) {
            *pixel_buffer_size = width * height;
            *pixel_buffer_size *= lv_color_format_get_size(LV_COLOR_FORMAT_NATIVE);
...
static void lv_windows_display_timer_callback(lv_timer_t *timer)
{
    ...
    lv_display_set_resolution(context->display_device_object, context->requested_display_resolution.x,
    ...
    if (window_handle) {
        ...
        context->display_framebuffer_context_handle = lv_windows_create_frame_buffer(
            window_handle, hor_res, ver_res, &context->display_framebuffer_base, &context->display_framebuffer_size);
        if (context->display_framebuffer_context_handle) {
            lv_display_set_buffers(context->display_device_object, context->display_framebuffer_base, NULL,
                                   (uint32_t)context->display_framebuffer_size, LV_DISPLAY_RENDER_MODE_DIRECT);
        }
    }
...
static void lv_windows_display_driver_flush_callback(lv_display_t *display, const lv_area_t *area, uint8_t *px_map)
{
    LV_UNUSED(area);
...
#if (LV_COLOR_DEPTH == 32) || (LV_COLOR_DEPTH == 24) || (LV_COLOR_DEPTH == 16)
...
#error[lv_windows] Unsupported LV_COLOR_DEPTH.
        ...
        if (hdc) {
            ...
            int32_t width = lv_windows_zoom_to_logical(client_rect.right - client_rect.left, context->zoom_level);
            ...
            StretchBlt(hdc, client_rect.left, client_rect.top, client_rect.right - client_rect.left,
                       client_rect.bottom - client_rect.top, context->display_framebuffer_context_handle, 0, 0, width,
                       height, SRCCOPY);
...
static UINT lv_windows_get_dpi_for_window(HWND window_handle)
{
    ...
    if (module_handle) {
        ...
        typedef HRESULT(WINAPI * function_type)(HMONITOR, MONITOR_DPI_TYPE_PRIVATE, UINT *, UINT *);

        function_type function = (function_type)(GetProcAddress(module_handle, "GetDpiForMonitor"));
        if (function) {
            HMONITOR MonitorHandle = MonitorFromWindow(window_handle, MONITOR_DEFAULTTONEAREST);

            UINT dpiX = 0;
            UINT dpiY = 0;
            if (SUCCEEDED(function(MonitorHandle, MDT_EFFECTIVE_DPI, &dpiX, &dpiY))) {
                result = dpiX;
            }
        }
...
static BOOL lv_windows_register_touch_window(HWND window_handle, ULONG flags)
{
    ...
    function_type function = (function_type)(GetProcAddress(module_handle, "RegisterTouchWindow"));
    if (!function) {
        return FALSE;
    }
...
static BOOL lv_windows_enable_child_window_dpi_message(HWND WindowHandle)
{
    ...
    if (!VerifyVersionInfoW(
            &os_version_info_ex, VER_MAJORVERSION | VER_MINORVERSION | VER_BUILDNUMBER,
    ...
    function_type function = (function_type)(GetProcAddress(module_handle, "EnableChildWindowDpiMessage"));
    if (!function) {
        return FALSE;
    }
                                                      ...
                                                      LRESULT *plResult)
{
    ...
    case WM_CREATE: {
        ...
        lv_windows_create_display_data_t *data =
            (lv_windows_create_display_data_t *)(((LPCREATESTRUCTW)(lParam))->lpCreateParams);
        if (!data) {
            return -1;
        }

        lv_windows_window_context_t *context = (lv_windows_window_context_t *)(HeapAlloc(
            GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(lv_windows_window_context_t)));
        if (!context) {
            return -1;
        }
        ...
        context->display_timer_object = lv_timer_create(lv_windows_display_timer_callback, LV_DEF_REFR_PERIOD, context);
        ...
        lv_display_set_resolution(context->display_device_object,
                                  request_content_size.right - request_content_size.left,
                                  request_content_size.bottom - request_content_size.top);
        lv_display_set_flush_cb(context->display_device_object, lv_windows_display_driver_flush_callback);
        lv_display_set_driver_data(context->display_device_object, hWnd);
        if (!context->allow_dpi_override) {
            lv_display_set_dpi(context->display_device_object, context->window_dpi);
        }

        if (context->simulator_mode) {
            context->display_resolution_changed = true;
            context->requested_display_resolution.x =
                lv_display_get_horizontal_resolution(context->display_device_object);
            context->requested_display_resolution.y =
                lv_display_get_vertical_resolution(context->display_device_object);
        }
                ...
                if (!context->simulator_mode) {
                    ...
                    int32_t window_width = lv_windows_dpi_to_physical(
                        lv_windows_zoom_to_physical(
                            lv_display_get_horizontal_resolution(context->display_device_object), context->zoom_level),
                        context->window_dpi);
                    int32_t window_height = lv_windows_dpi_to_physical(
                        lv_windows_zoom_to_physical(lv_display_get_vertical_resolution(context->display_device_object),
                                                    context->zoom_level),
                        context->window_dpi);
                    ...
                    int32_t original_window_width = window_rect.right - window_rect.left;
                    int32_t original_window_height = window_rect.bottom - window_rect.top;

                    int32_t original_client_width = client_rect.right - client_rect.left;
                    int32_t original_client_height = client_rect.bottom - client_rect.top;

                    int32_t reserved_width = original_window_width - original_client_width;
                    int32_t reserved_height = original_window_height - original_client_height;

                    SetWindowPos(hWnd, NULL, 0, 0, reserved_width + window_width, reserved_height + window_height,
    ...
    case WM_DPICHANGED: {
        ...
        if (context) {
            ...
            SetWindowPos(hWnd, NULL, suggested_rect->left, suggested_rect->top, suggested_rect->right,
                         suggested_rect->bottom, SWP_NOZORDER | SWP_NOACTIVATE);
        }
    ...
    case WM_DESTROY: {
        lv_windows_window_context_t *context =
            (lv_windows_window_context_t *)(RemovePropW(hWnd, L"LVGL.Window.Context"));
        if (context) {
    ...
    default: {
        ...
        if (context) {
            if (context->pointer.indev &&
                lv_windows_pointer_device_window_message_handler(hWnd, uMsg, wParam, lParam, plResult)) {
                // Handled
                return true;
            } else if (context->keypad.indev &&
                       lv_windows_keypad_device_window_message_handler(hWnd, uMsg, wParam, lParam, plResult)) {
                // Handled
                return true;
            } else if (context->encoder.indev &&
                       lv_windows_encoder_device_window_message_handler(hWnd, uMsg, wParam, lParam, plResult)) {
                // Handled
                return true;
            }
        }
...
static LRESULT CALLBACK lv_windows_window_message_callback(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    LRESULT lResult = 0;
    bool Handled = lv_windows_window_message_callback_nolock(hWnd, uMsg, wParam, lParam, &lResult);
