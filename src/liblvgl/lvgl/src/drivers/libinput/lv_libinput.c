...
static bool _rescan_devices(void)
{
    ...
    while ((ent = readdir(dir))) {
        ...
        /* The device pointer is guaranteed to be valid until the next libinput_dispatch. Since we're not dispatching
         * events as part of this function, we don't have to increase its reference count to keep it alive.
...
static void _read_pointer(lv_libinput_t *dsc, struct libinput_event *event)
{
    ...
    case LIBINPUT_EVENT_TOUCH_DOWN: {
        lv_point_t point;
        point.x = (int32_t)LV_CLAMP(
            INT32_MIN, libinput_event_touch_get_x_transformed(touch_event, hor_res) - disp->offset_x, INT32_MAX);
        point.y = (int32_t)LV_CLAMP(
            INT32_MIN, libinput_event_touch_get_y_transformed(touch_event, ver_res) - disp->offset_y, INT32_MAX);
    ...
    case LIBINPUT_EVENT_POINTER_MOTION_ABSOLUTE: {
        lv_point_t point;
        point.x = (int32_t)LV_CLAMP(
            INT32_MIN, libinput_event_pointer_get_absolute_x_transformed(pointer_event, hor_res) - disp->offset_x,
            INT32_MAX);
        point.y = (int32_t)LV_CLAMP(
            INT32_MIN, libinput_event_pointer_get_absolute_y_transformed(pointer_event, ver_res) - disp->offset_y,
            INT32_MAX);
        if (point.x < 0 || point.x > disp->hor_res || point.y < 0 || point.y > disp->ver_res) {
    ...
    case LIBINPUT_EVENT_POINTER_BUTTON: {
        ...
        dsc->pointer_button_down =
            button_state == LIBINPUT_BUTTON_STATE_RELEASED ? LV_INDEV_STATE_RELEASED : LV_INDEV_STATE_PRESSED;
        evt->point.x = dsc->pointer_position.x;
        evt->point.y = dsc->pointer_position.y;
        evt->pressed = dsc->pointer_button_down;
    }
...
static void _read_keypad(lv_libinput_t *dsc, struct libinput_event *event)
{
    ...
    case LIBINPUT_EVENT_KEYBOARD_KEY:
        ...
        if (evt->key_val != 0) {
            ...
            evt->pressed =
                (key_state == LIBINPUT_KEY_STATE_RELEASED) ? LV_INDEV_STATE_RELEASED : LV_INDEV_STATE_PRESSED;

            // just release the key immediately after it got pressed.
            // but don't handle special keys where holding a key makes sense
            if (evt->key_val != LV_KEY_BACKSPACE && evt->key_val != LV_KEY_UP && evt->key_val != LV_KEY_LEFT &&
                evt->key_val != LV_KEY_RIGHT && evt->key_val != LV_KEY_DOWN &&
