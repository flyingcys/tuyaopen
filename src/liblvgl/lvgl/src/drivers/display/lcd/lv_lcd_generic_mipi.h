...
#define LV_LCD_CMD_GET_ADDRESS_MODE                                                                                    \
    0x0B /* Get the data order for transfers from the Host to the display module and from the frame memory to the      \
            display device */
...
#define LV_LCD_CMD_WRITE_MEMORY_START                                                                                  \
    0x2C /* Transfer image data from the Host Processor to the peripheral starting at the location provided by         \
            set_column_address and set_page_address */
#define LV_LCD_CMD_READ_MEMORY_START                                                                                   \
    0x2E /* Transfer image data from the peripheral to the Host Processor interface starting at the location provided  \
            by set_column_address and set_page_address */
#define LV_LCD_CMD_SET_PARTIAL_ROWS                                                                                    \
    0x30 /* Defines the number of rows in the partial display area on the display device */
#define LV_LCD_CMD_SET_PARTIAL_COLUMNS                                                                                 \
    0x31 /* Defines the number of columns in the partial display area on the display device */
#define LV_LCD_CMD_SET_SCROLL_AREA 0x33 /* Defines the vertical scrolling and fixed area on display device */
#define LV_LCD_CMD_SET_TEAR_OFF                                                                                        \
    0x34 /* Synchronization information is not sent from the display module to the host processor */
#define LV_LCD_CMD_SET_TEAR_ON                                                                                         \
    0x35 /* Synchronization information is sent from the display module to the host processor at the start of VFP */
#define LV_LCD_CMD_SET_ADDRESS_MODE                                                                                    \
    0x36 /* Set the data order for transfers from the Host to the display module and from the frame memory to the      \
            display device */
...
#define LV_LCD_CMD_WRITE_MEMORY_CONTINUE                                                                               \
    0x3C /* Transfer image information from the Host Processor interface to the peripheral from the last written       \
            location */
#define LV_LCD_CMD_READ_MEMORY_CONTINUE                                                                                \
    0x3E /* Read image data from the peripheral continuing after the last read_memory_continue or read_memory_start */
#define LV_LCD_CMD_SET_TEAR_SCANLINE                                                                                   \
    0x44 /* Synchronization information is sent from the display module to the host processor when the display device  \
...
#define LV_LCD_MASK_FLIP_VERTICAL                                                                                      \
    (1 << 0) /* This bit flips the image shown on the display device top to bottom. No change is made to the frame     \
                memory */
#define LV_LCD_MASK_FLIP_HORIZONTAL                                                                                    \
    (1 << 1) /* This bit flips the image shown on the display device left to right. No change is made to the frame     \
                memory */
...
#define LV_LCD_BIT_FLIP_VERTICAL__FLIPPED                                                                              \
    LV_LCD_MASK_FLIP_VERTICAL /* This bit flips the image shown on the display device top to bottom. No change is made \
                                 to the frame memory */
#define LV_LCD_BIT_FLIP_HORIZONTAL__NOT_FLIPPED 0
#define LV_LCD_BIT_FLIP_HORIZONTAL__FLIPPED                                                                            \
    LV_LCD_MASK_FLIP_HORIZONTAL /* This bit flips the image shown on the display device left to right. No change is    \
                                   made to the frame memory */
#define LV_LCD_BIT_DATA_LATCH_DATA_ORDER__LTOR 0 /* Display Data Latch Order: LCD Refresh Left to Right */
#define LV_LCD_BIT_DATA_LATCH_DATA_ORDER__RTOL                                                                         \
    LV_LCD_MASK_DATA_LATCH_DATA_ORDER         /* Display Data Latch Order: LCD Refresh Right to Left */
...
#define LV_LCD_BIT_LINE_ADDRESS_ORDER__BTOT                                                                            \
    LV_LCD_MASK_LINE_ADDRESS_ORDER              /* Line Address Order: LCD Refresh Bottom to Top */
...
#define LV_LCD_BIT_COLUMN_ADDRESS_ORDER__RTOL                                                                          \
    LV_LCD_MASK_COLUMN_ADDRESS_ORDER                                       /* Column Address Order: Right to Left      \
                                                                            */
 ...
 * @param send_cmd      platform-dependent function to send a command to the LCD controller (usually uses polling
 * transfer)
 * @param send_color    platform-dependent function to send pixel data to the LCD controller (usually uses DMA transfer:
 * must implement a 'ready' callback)
