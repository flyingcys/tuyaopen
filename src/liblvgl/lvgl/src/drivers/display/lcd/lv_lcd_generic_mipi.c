...
{
    lv_lcd_generic_mipi_driver_t *drv = get_driver(disp);
    send_cmd(drv, LV_LCD_CMD_SET_GAMMA_CURVE,
             (uint8_t[]){
                 gamma,
             },
             1);
...
static void init(lv_lcd_generic_mipi_driver_t *drv, lv_lcd_flag_t flags)
{
    ...
    send_cmd(drv, LV_LCD_CMD_SET_ADDRESS_MODE,
             (uint8_t[]){
                 drv->madctl_reg,
             },
             1);
    send_cmd(drv, LV_LCD_CMD_SET_PIXEL_FORMAT,
             (uint8_t[]){
                 drv->colmod_reg,
             },
             1);
 ...
 * @note                transfers pixel data to the LCD controller using the callbacks 'send_cmd' and 'send_color',
 * which were passed to the 'lv_st7789_create()' function
 */
static void flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map)
{
    lv_lcd_generic_mipi_driver_t *drv = get_driver(disp);
    ...
    send_cmd(drv, LV_LCD_CMD_SET_COLUMN_ADDRESS,
             (uint8_t[]){
                 (x_start >> 8) & 0xFF,
                 x_start & 0xFF,
                 ((x_end - 1) >> 8) & 0xFF,
             ...
             4);
    send_cmd(drv, LV_LCD_CMD_SET_PAGE_ADDRESS,
             (uint8_t[]){
                 (y_start >> 8) & 0xFF,
                 y_start & 0xFF,
                 ((y_end - 1) >> 8) & 0xFF,
...
static void set_rotation(lv_lcd_generic_mipi_driver_t *drv, lv_display_rotation_t rot)
{
    ...
    send_cmd(drv, LV_LCD_CMD_SET_ADDRESS_MODE, (uint8_t[]){drv->madctl_reg}, 1);
}
