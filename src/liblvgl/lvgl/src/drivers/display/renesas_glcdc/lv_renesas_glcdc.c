...
#ifdef _RENESAS_RX_
static uint8_t fb_background[2][LCD_CH0_IN_GR2_HSIZE * LCD_CH0_IN_GR2_VSIZE * BYTES_PER_PIXEL]
    __attribute__((section(".framebuffer"), aligned(64), used));
...
static void glcdc_init(void)
{
    ...
    do {
        err = R_GLCDC_BufferChange(&g_display0_ctrl, (uint8_t *)p_fb, (display_frame_layer_t)0);
...
#ifdef _RENESAS_RA_
    R_GLCDC_BufferChange(&g_display0_ctrl, (uint8_t *)px_map, (display_frame_layer_t)0);
#else  /* RX */
    glcdc_err_t err;
