...
#define LL_FOREACH(entry, head, member)                                                                                \
    for ((entry) = LL_FIRST(head); (entry) != NULL; (entry) = LL_NEXT(entry, member))
...
void *smm_map(smm_buffer_t *buf)
{
    ...
    if (pool->map_outdated) {
        ...
        map = mmap(NULL, pool->props.size, PROT_READ | PROT_WRITE, MAP_SHARED, pool->props.fd, 0);
...
void smm_release(smm_buffer_t *buf)
{
    ...
    if (rbuf->group_resized) {
        ...
        while ((grp->num_buffers > PREFER_NUM_BUFFERS) && (!LL_IS_EMPTY(&grp->unused))) {
    ...
    if (smm_instance.active == NULL) {
        ...
        LL_FOREACH(buf, &smm_instance.active->allocd, pool)
        {
            ...
            if (buf->props.group == NULL) {
                ...
                if (buf_sz == grp->size) {
                    ...
                    if ((buf != LL_LAST(&smm_instance.active->allocd)) && (LL_NEXT(buf, pool)->props.group == NULL)) {
        ...
        if (buf == NULL) {
            ...
            if (last != NULL) {
                ...
                ret = ftruncate(smm_instance.active->props.fd, smm_instance.active->props.size + buf_sz);
                if (ret) {
                    if (buf != NULL) {
                        free_buffer(buf);
                        buf = NULL;
                    ...
                    if (!(smm_instance.active->props.size - buf_sz)) {
                        ...
                        if ((smm_instance.cbs.new_pool != NULL) &&
                            (smm_instance.cbs.new_pool(smm_instance.cbs.ctx, &smm_instance.active->props))) {
                            free_buffer(buf);
                        ...
                        if (smm_instance.cbs.expand_pool != NULL) {
                            smm_instance.cbs.expand_pool(smm_instance.cbs.ctx, &smm_instance.active->props);
                        }
                    }
                }
            }
...
void return_to_pool(struct smm_buffer *buf)
{
    ...
    if ((buf != LL_LAST(&pool->allocd)) && (LL_NEXT(buf, pool)->props.group == NULL)) {
        free_buffer(LL_NEXT(buf, pool));
    }
    if ((buf != LL_FIRST(&pool->allocd)) && (LL_PREV(buf, pool)->props.group == NULL)) {
    ...
    if ((buf == LL_FIRST(&pool->allocd)) && (buf == LL_LAST(&pool->allocd))) {
        free_buffer(buf);
...
struct smm_pool *alloc_pool(void)
{
    ...
    if (pool != NULL) {
        do {
            ...
            pool->props.fd = shm_open(name, O_RDWR | O_CREAT | O_EXCL, S_IRUSR | S_IWUSR);
            if (pool->props.fd >= 0) {
                shm_unlink(name);
                pool->props.size = 0;
                pool->map = NULL;
...
{
    struct smm_buffer *buf;
    struct smm_buffer_properties initial_props = {{NULL}, NULL, smm_instance.active, offset};
