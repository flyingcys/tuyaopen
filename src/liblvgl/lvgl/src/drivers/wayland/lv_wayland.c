...
static struct graphic_object *create_graphic_obj(struct application *app, struct window *window, enum object_type type,
...
static void wl_shell_handle_configure(void *data, struct wl_shell_surface *shell_surface, uint32_t edges, int32_t width,
                                      int32_t height)
{
...
static void xdg_toplevel_handle_configure(void *data, struct xdg_toplevel *xdg_toplevel, int32_t width, int32_t height,
...
static void xdg_toplevel_handle_configure_bounds(void *data, struct xdg_toplevel *xdg_toplevel, int32_t width,
                                                 int32_t height)
{
...
#if LV_WAYLAND_WINDOW_DECORATIONS
static bool attach_decoration(struct window *window, struct graphic_object *decoration, smm_buffer_t *decoration_buffer,
                              struct graphic_object *parent)
{
...
static bool create_decoration(struct window *window, struct graphic_object *decoration, int window_width,
                              int window_height)
{
    smm_buffer_t *buf;
    void *buf_base;
...
static void _lv_wayland_flush(lv_display_t *disp, const lv_area_t *area, unsigned char *color_p)
{
    ...
    for (y = area->y1; y <= area->y2; y++) {
        ...
        memcpy(((char *)buf_base) + offset, color_p, src_width * bpp);
...
static void wayland_init(void)
{
    struct smm_events evs = {NULL,           sme_new_pool,    sme_expand_pool, sme_free_pool,
