...
uint32_t lv_nuttx_get_idle(void)
{
    ...
    LV_LOG_TRACE("active = %" LV_PRIu32 ", total = %" LV_PRIu32, cpuload.active, cpuload.total);
...
static void syslog_print(lv_log_level_t level, const char *buf)
{
    static const int priority[LV_LOG_LEVEL_NUM] = {LOG_DEBUG, LOG_INFO, LOG_WARNING, LOG_ERR, LOG_CRIT};
...
static void check_stack_size(void)
{
    ...
    if (stack_size < LV_NUTTX_MIN_STACK_SIZE) {
        LV_LOG_ERROR("Stack size is too small. Please increase it to %d bytes or more.", LV_NUTTX_MIN_STACK_SIZE);
    }
}
