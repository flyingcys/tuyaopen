...
static void x11_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map)
{
    ...
    color_t *src_data =
        (color_t *)px_map + (LV_X11_RENDER_MODE == LV_DISPLAY_RENDER_MODE_PARTIAL ? 0 : hor_res * area->y1 + area->x1);
    ...
    if (lv_display_flush_is_last(disp)) {
        ...
        XPutImage(xd->hdr.display, xd->window, xd->gc, xd->ximage, xd->flush_area.x1, xd->flush_area.y1,
                  xd->flush_area.x1, xd->flush_area.y1, upd_w, upd_h);
...
static void x11_resolution_evt_cb(lv_event_t *e)
{
    ...
    xd->ximage = XCreateImage(xd->hdr.display, xd->visual, xd->dplanes, ZPixmap, 0, xd->xdata, hor_res, ver_res,
                              lv_color_format_get_bpp(LV_COLOR_FORMAT_ARGB8888), 0);
}
    ...
    while (XCheckIfEvent(xd->hdr.display, &event, is_disp_event, NULL)) {
        ...
        case Expose:
            if (event.xexpose.count == 0) {
                XPutImage(xd->hdr.display, xd->window, xd->gc, xd->ximage, 0, 0, 0, 0, event.xexpose.width,
...
#else
    xd->window = XCreateWindow(xd->hdr.display, XDefaultRootWindow(xd->hdr.display), 0, 0, hor_res, ver_res, 0,
                               XDefaultDepth(xd->hdr.display, screen), InputOutput, xd->visual, 0, NULL);
#endif
