...
lv_draw_task_t *lv_draw_get_next_available_task(lv_layer_t *layer, lv_draw_task_t *t_prev, uint8_t draw_unit_id)
{
    ...
    if (_draw_info.unit_cnt <= 1) {
        ...
        while (t) {
            ...
            if (t->state == LV_DRAW_TASK_STATE_QUEUED && t->preferred_draw_unit_id != LV_DRAW_UNIT_NONE &&
    ...
    if (layer->draw_task_head) {
        ...
        lv_draw_task_t *t = layer->draw_task_head;
        if (t->state != LV_DRAW_TASK_STATE_QUEUED && t->area.x1 <= 0 && t->area.x2 >= hor_res - 1 && t->area.y1 <= 0 &&
            t->area.y2 >= ver_res - 1) {
            LV_PROFILER_END;
            return NULL;
        }
