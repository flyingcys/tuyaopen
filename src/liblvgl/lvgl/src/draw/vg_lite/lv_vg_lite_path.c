...
lv_vg_lite_path_t *lv_vg_lite_path_create(vg_lite_format_t data_format)
{
    ...
    LV_ASSERT(vg_lite_init_path(&path->base, data_format, VG_LITE_MEDIUM, 0, NULL, 0, 0, 0, 0) == VG_LITE_SUCCESS);
    LV_PROFILER_END;
    return path;
}
...
void lv_vg_lite_path_set_bonding_box(lv_vg_lite_path_t *path, float min_x, float min_y, float max_x, float max_y)
{
    LV_ASSERT_NULL(path);
...
void lv_vg_lite_path_get_bonding_box(lv_vg_lite_path_t *path, float *min_x, float *min_y, float *max_x, float *max_y)
{
    LV_ASSERT_NULL(path);
    if (min_x)
...
void lv_vg_lite_path_append_arc_right_angle(lv_vg_lite_path_t *path, float start_x, float start_y, float center_x,
                                            float center_y, float end_x, float end_y)
{
    LV_PROFILER_BEGIN;
    ...
    lv_vg_lite_path_cubic_to(path, start_x - c * dy1, start_y + c * dx1, end_x - c * dy2, end_y + c * dx2, end_x,
                             end_y);
    LV_PROFILER_END;
}

void lv_vg_lite_path_append_arc(lv_vg_lite_path_t *path, float cx, float cy, float radius, float start_angle,
