...
void lv_draw_vg_lite_line(lv_draw_unit_t *draw_unit, const lv_draw_line_dsc_t *dsc)
{
    ...
    */
    ...
    if (dsc->round_start) {
        ...
        /* start 90deg arc */
        lv_vg_lite_path_append_arc_right_angle(path, head_start_x, head_start_y, p1_x, p1_y, arc_cx, arc_cy);

        /* end 90deg arc */
        lv_vg_lite_path_append_arc_right_angle(path, arc_cx, arc_cy, p1_x, p1_y, head_end_x, head_end_y);
    } else {
    ...
    if (dsc->round_end) {
        ...
        lv_vg_lite_path_append_arc_right_angle(path, tail_start_x, tail_start_y, p2_x, p2_y, arc_cx, arc_cy);
        lv_vg_lite_path_append_arc_right_angle(path, arc_cx, arc_cy, p2_x, p2_y, tail_end_x, tail_end_y);
    } else {
    ...
    for (int32_t i = 0; i < ndash; i++) {
        ...
        lv_vg_lite_path_line_to(path, p1_x + w2_dx + dx * (i * dash_l + dash_width) * inv_dl,
                                p1_y - w2_dy + dy * (i * dash_l + dash_width) * inv_dl);
        lv_vg_lite_path_line_to(path, p1_x + w2_dx + dx * (i + 1) * dash_l * inv_dl,
    ...
    LV_VG_LITE_CHECK_ERROR(
        vg_lite_draw(&u->target_buffer, vg_lite_path, VG_LITE_FILL_EVEN_ODD, &matrix, VG_LITE_BLEND_SRC_OVER, color));
