...
lv_cache_entry_t *lv_vg_lite_stroke_get(struct lv_draw_vg_lite_unit_t *unit, struct lv_vg_lite_path_t *path,
...
static bool stroke_create_cb(stroke_item_t *item, void *user_data)
{
    ...
    vg_lite_error_t error =
        vg_lite_set_stroke(vg_path, lv_stroke_cap_to_vg(item->cap), lv_stroke_join_to_vg(item->join), item->width,
                           item->miter_limit, lv_array_front(&item->dash_pattern), size, item->width / 2, 0);
