...
void lv_draw_vg_lite_arc(lv_draw_unit_t *draw_unit, const lv_draw_arc_dsc_t *dsc, const lv_area_t *coords)
{
    ...
    if (math_equal(sweep_angle, 360)) {
        ...
        /* radius_out arc */
        lv_vg_lite_path_append_arc(path, cx, cy, radius_out, start_angle, sweep_angle, false);

        /* line to radius_in */
        lv_vg_lite_path_line_to(path, end_x, end_y);

        /* radius_in arc */
        lv_vg_lite_path_append_arc(path, cx, cy, radius_in, end_angle, -sweep_angle, false);
    ...
    LV_VG_LITE_CHECK_ERROR(vg_lite_draw(&u->target_buffer, vg_lite_path, fill, &matrix, VG_LITE_BLEND_SRC_OVER, color));
    ...
    if (dsc->img_src) {
        ...
        if (lv_vg_lite_buffer_open_image(&src_buf, &decoder_dsc, dsc->img_src, false, true)) {
            ...
            LV_VG_LITE_CHECK_ERROR(vg_lite_draw_pattern(&u->target_buffer, vg_lite_path, fill, &path_matrix, &src_buf,
                                                        &matrix, VG_LITE_BLEND_SRC_OVER, VG_LITE_PATTERN_COLOR, 0,
                                                        color, VG_LITE_FILTER_BI_LINEAR));
