                          ...
                          const vg_lite_matrix_t *matrix, vg_lite_fill_t fill, vg_lite_blend_t blend)
{
    ...
    case GRAD_TYPE_LINEAR: {
        ...
        LV_VG_LITE_CHECK_ERROR(vg_lite_draw_grad(buffer, path, fill, (vg_lite_matrix_t *)matrix, linear_grad, blend));
    ...
    case GRAD_TYPE_LINEAR_EXT: {
        ...
        *grad_mat_p = *grad_matrix;
        ...
        LV_VG_LITE_CHECK_ERROR(vg_lite_draw_linear_grad(buffer, path, fill, (vg_lite_matrix_t *)matrix, linear_grad, 0,
                                                        blend, VG_LITE_FILTER_LINEAR));
    ...
    case GRAD_TYPE_RADIAL: {
        ...
        *grad_mat_p = *grad_matrix;
        ...
        LV_VG_LITE_CHECK_ERROR(vg_lite_draw_radial_grad(buffer, path, fill, (vg_lite_matrix_t *)matrix,
                                                        &grad_item->vg.radial, 0, blend, VG_LITE_FILTER_LINEAR));
...
static bool linear_ext_grad_create(grad_item_t *item)
{
    ...
    LV_VG_LITE_CHECK_ERROR(vg_lite_set_linear_grad(&linear_grad, item->lv.stops_count, color_ramp, grad_param,
                                                   lv_spread_to_vg(item->lv.spread), 1));
...
static bool radial_grad_create(grad_item_t *item)
{
    ...
    LV_VG_LITE_CHECK_ERROR(vg_lite_set_radial_grad(&radial_grad, item->lv.stops_count, color_ramp, grad_param,
                                                   lv_spread_to_vg(item->lv.spread), 1));
...
static lv_cache_compare_res_t grad_compare_cb(const grad_item_t *lhs, const grad_item_t *rhs)
{
    ...
    int cmp_res = lv_memcmp(lhs->lv.stops, rhs->lv.stops, sizeof(lv_gradient_stop_t) * lhs->lv.stops_count);
    if (cmp_res != 0) {
