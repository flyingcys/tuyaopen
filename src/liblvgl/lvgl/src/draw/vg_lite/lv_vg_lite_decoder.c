...
static void image_decode_to_index8_line(uint8_t *dest, const uint8_t *src, int32_t w_px, lv_color_format_t color_format)
{
    uint8_t px_size;
    uint16_t mask;
...
static lv_result_t decoder_open_variable(lv_image_decoder_t *decoder, lv_image_decoder_dsc_t *dsc)
{
    ...
    lv_draw_buf_t *draw_buf =
        lv_draw_buf_create_ex(image_cache_draw_buf_handlers, width, height, DEST_IMG_FORMAT, LV_STRIDE_AUTO);
    if (draw_buf == NULL) {
        return LV_RESULT_INVALID;
    }
...
static lv_result_t decoder_open_file(lv_image_decoder_t *decoder, lv_image_decoder_dsc_t *dsc)
{
    ...
    if (res != LV_FS_RES_OK || palette_br != palette_size_bytes) {
        LV_LOG_ERROR("read %s (palette: %" LV_PRIu32 ", br: %" LV_PRIu32 ") failed", path, palette_size_bytes,
                     palette_br);
        goto failed;
    }
    ...
    for (uint32_t y = 0; y < height; y++) {
        ...
        if (res != LV_FS_RES_OK || br != src_stride) {
            LV_LOG_ERROR("read %s (y: %" LV_PRIu32 ", src_stride: %" LV_PRIu32 ", br: %" LV_PRIu32 ") failed", path, y,
                         src_stride, br);
            goto failed;
        }
