...
static void task_draw_cb(void *ctx, const lv_vector_path_t *path, const lv_vector_draw_dsc_t *dsc)
{
    ...
    if (path_type == VG_LITE_DRAW_ZERO || path_type == VG_LITE_DRAW_FILL_PATH ||
        path_type == VG_LITE_DRAW_FILL_STROKE_PATH) {
        lv_vg_lite_path_end(lv_vg_path);
    }

    /* convert stroke style */
    if (path_type == VG_LITE_DRAW_STROKE_PATH || path_type == VG_LITE_DRAW_FILL_STROKE_PATH) {
    ...
    case LV_VECTOR_DRAW_STYLE_SOLID: {
        ...
        LV_VG_LITE_CHECK_ERROR(vg_lite_draw(&u->target_buffer, vg_path, fill, &matrix, blend, vg_color));
    ...
    case LV_VECTOR_DRAW_STYLE_PATTERN: {
        ...
        if (lv_vg_lite_buffer_open_image(&image_buffer, &decoder_dsc, dsc->fill_dsc.img_dsc.src, false, true)) {
            ...
            vg_lite_color_t recolor =
                lv_vg_lite_color(dsc->fill_dsc.img_dsc.recolor, dsc->fill_dsc.img_dsc.recolor_opa, true);

            LV_VG_LITE_ASSERT_MATRIX(&pattern_matrix);

            LV_PROFILER_BEGIN_TAG("vg_lite_draw_pattern");
            LV_VG_LITE_CHECK_ERROR(vg_lite_draw_pattern(&u->target_buffer, vg_path, fill, &matrix, &image_buffer,
                                                        &pattern_matrix, blend, VG_LITE_PATTERN_COLOR, vg_color,
                                                        recolor, VG_LITE_FILTER_BI_LINEAR));
    ...
    case LV_VECTOR_DRAW_STYLE_GRADIENT: {
        ...
        lv_vg_lite_draw_grad(u, &u->target_buffer, vg_path, &dsc->fill_dsc.gradient, &grad_matrix, &matrix, fill,
                             blend);
    } break;
    default:
