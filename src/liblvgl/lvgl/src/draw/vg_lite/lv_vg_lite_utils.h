...
#define LV_VG_LITE_CHECK_ERROR(expr)                                                                                   \
    do {                                                                                                               \
        ...
        if (LV_VG_LITE_IS_ERROR(error)) {                                                                              \
            LV_LOG_ERROR("Execute '" #expr "' error(%d): %s", (int)error, lv_vg_lite_error_string(error));             \
...
#define LV_VG_LITE_ALIGN(number, align_bytes) (((number) + ((align_bytes)-1)) & ~((align_bytes)-1))

#define LV_VG_LITE_IS_ALIGNED(num, align) (((uintptr_t)(num) & ((align)-1)) == 0)

#define LV_VG_LITE_IS_INDEX_FMT(fmt)                                                                                   \
    ((fmt) == VG_LITE_INDEX_1 || (fmt) == VG_LITE_INDEX_2 || (fmt) == VG_LITE_INDEX_4 || (fmt) == VG_LITE_INDEX_8)
