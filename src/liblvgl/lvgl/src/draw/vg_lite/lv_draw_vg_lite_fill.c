...
void lv_draw_vg_lite_fill(lv_draw_unit_t *draw_unit, const lv_draw_fill_dsc_t *dsc, const lv_area_t *coords)
{
    ...
    lv_vg_lite_path_append_rect(path, coords->x1, coords->y1, lv_area_get_width(coords), lv_area_get_height(coords),
                                dsc->radius);
    ...
    if (dsc->grad.dir != LV_GRAD_DIR_NONE) {
        ...
        lv_vg_lite_draw_grad_helper(u, &u->target_buffer, vg_lite_path, coords, &dsc->grad, &matrix,
                                    VG_LITE_FILL_EVEN_ODD, VG_LITE_BLEND_SRC_OVER);
#else
        ...
        LV_VG_LITE_CHECK_ERROR(vg_lite_draw(&u->target_buffer, vg_lite_path, VG_LITE_FILL_EVEN_ODD, &matrix,
