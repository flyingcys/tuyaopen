...
void lv_vg_lite_dump_info(void)
{
    ...
    LV_LOG_USER("Product Info: %s"
                " | Chip ID: 0x%" LV_PRIx32 " | Revision: 0x%" LV_PRIx32 " | CID: 0x%" LV_PRIx32,
    ...
    for (int feature = 0; feature < gcFEATURE_COUNT; feature++) {
        ...
        LV_UNUSED(ret);
        LV_LOG_USER("Feature-%d: %s\t - %s", feature, lv_vg_lite_feature_string((vg_lite_feature_t)feature),
                    ret ? "YES" : "NO");
    }
...
void lv_vg_lite_path_dump_info(const vg_lite_path_t *path)
{
    ...
    LV_LOG_USER("bonding box: (%0.2f, %0.2f) - (%0.2f, %0.2f)", path->bounding_box[0], path->bounding_box[1],
...
void lv_vg_lite_buffer_dump_info(const vg_lite_buffer_t *buffer)
{
    ...
    LV_LOG_USER("format: %d (%s)", (int)((buffer)->format), lv_vg_lite_buffer_format_string((buffer)->format));
...
{
    for (int i = 0; i < 3; i++) {
        LV_LOG_USER("| %f, %f, %f |", (matrix)->m[i][0], (matrix)->m[i][1], (matrix)->m[i][2]);
    }
}
...
void lv_vg_lite_buffer_init(vg_lite_buffer_t *buffer, const void *ptr, int32_t width, int32_t height, uint32_t stride,
                            vg_lite_buffer_format_t format, bool tiled)
{
    uint32_t mul;
    uint32_t div;
    uint32_t align;
...
void lv_vg_lite_image_matrix(vg_lite_matrix_t *matrix, int32_t x, int32_t y, const lv_draw_image_dsc_t *dsc)
{
    ...
    if (rotation != 0 || scale_x != LV_SCALE_NONE || scale_y != LV_SCALE_NONE) {
        ...
        if (scale_x != LV_SCALE_NONE || scale_y != LV_SCALE_NONE) {
            vg_lite_scale((vg_lite_float_t)scale_x / LV_SCALE_NONE, (vg_lite_float_t)scale_y / LV_SCALE_NONE, matrix);
        }
...
bool lv_vg_lite_buffer_check(const vg_lite_buffer_t *buffer, bool is_src)
{
    ...
    case VG_LITE_IMAGE_TRANSPARENT:
        break;
    default:
        LV_LOG_ERROR("buffer transparency_mode(%d) is invalid", (int)buffer->transparency_mode);
        return false;
    }
...
void lv_vg_lite_matrix_multiply(vg_lite_matrix_t *matrix, const vg_lite_matrix_t *mult)
{
    ...
    for (row = 0; row < 3; row++) {
        ...
        for (column = 0; column < 3; column++) {
            ...
            temp.m[row][column] =
                (m[row][0] * mult->m[0][column]) + (m[row][1] * mult->m[1][column]) + (m[row][2] * mult->m[2][column]);
        }
    }
...
#endif
    LV_VG_LITE_CHECK_ERROR(vg_lite_set_scissor(area->x1, area->y1, area->x2 + 1, area->y2 + 1));
}

void lv_vg_lite_disable_scissor(void)
{
    /* Restore full screen scissor */
    LV_VG_LITE_CHECK_ERROR(vg_lite_set_scissor(0, 0, LV_HOR_RES, LV_VER_RES));
}
