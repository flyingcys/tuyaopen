                          ...
                          const lv_area_t *img_coords, const lv_area_t *clipped_img_area)
{
    ...
    bool transformed =
        draw_dsc->rotation != 0 || draw_dsc->scale_x != LV_SCALE_NONE || draw_dsc->scale_y != LV_SCALE_NONE ? true
                                                                                                            : false;
    ...
    if (LV_COLOR_FORMAT_RGB565A8 == cf) {
        ...
        d2_framebuffer(u->d2_handle, p_intermediate_buf, (d2_s32)header->w, (d2_u32)header->w, (d2_u32)header->h,
                       lv_draw_dave2d_lv_colour_fmt_to_d2_fmt(LV_COLOR_FORMAT_ARGB8888));

        d2_cliprect(u->d2_handle, (d2_border)0, (d2_border)0, (d2_border)header->w - 1, (d2_border)header->h - 1);
        ...
        d2_settexturemapping(u->d2_handle, D2_FIX4(p1[0].x), D2_FIX4(p1[0].y), D2_FIX16(0), D2_FIX16(0), dxu1, dxv1,
                             dyu1, dyv1);
        d2_settexturemode(u->d2_handle, d2_tm_filter);
        d2_setfillmode(u->d2_handle, d2_fm_texture);

        d2_settexture(u->d2_handle, (void *)src_buf,
                      (d2_s32)(img_stride / lv_color_format_get_size(LV_COLOR_FORMAT_RGB565)), header->w, header->h,
                      lv_draw_dave2d_lv_colour_fmt_to_d2_fmt(LV_COLOR_FORMAT_RGB565));

        d2_setblendmode(u->d2_handle, d2_bm_one, d2_bm_zero);

        d2_renderquad(u->d2_handle, (d2_point)D2_FIX4(p1[0].x), (d2_point)D2_FIX4(p1[0].y), (d2_point)D2_FIX4(p1[1].x),
                      (d2_point)D2_FIX4(p1[1].y), (d2_point)D2_FIX4(p1[2].x), (d2_point)D2_FIX4(p1[2].y),
                      (d2_point)D2_FIX4(p1[3].x), (d2_point)D2_FIX4(p1[3].y), 0);
        ...
        d2_settexture(u->d2_handle,
                      (void *)(src_buf + header->h * (header->w * lv_color_format_get_size(LV_COLOR_FORMAT_RGB565))),
                      (d2_s32)(img_stride / lv_color_format_get_size(LV_COLOR_FORMAT_RGB565)), header->w, header->h,
                      lv_draw_dave2d_lv_colour_fmt_to_d2_fmt(LV_COLOR_FORMAT_A8));
    ...
    d2_settexture(u->d2_handle, (void *)src_buf, (d2_s32)(img_stride / lv_color_format_get_size(cf)), header->w,
                  header->h, lv_draw_dave2d_lv_colour_fmt_to_d2_fmt(cf));
    ...
    d2_renderquad(u->d2_handle, (d2_point)D2_FIX4(p[0].x), (d2_point)D2_FIX4(p[0].y), (d2_point)D2_FIX4(p[1].x),
                  (d2_point)D2_FIX4(p[1].y), (d2_point)D2_FIX4(p[2].x), (d2_point)D2_FIX4(p[2].y),
