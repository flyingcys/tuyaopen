...
void lv_draw_dave2d_triangle(lv_draw_dave2d_unit_t *u, const lv_draw_triangle_dsc_t *dsc)
{
    ...
    if (!lv_area_intersect(&clipped_area, &tri_area, u->base_unit.clip_area))
        return;
    ...
    d2_rendertri(u->d2_handle, (d2_point)D2_FIX4(p[0].x), (d2_point)D2_FIX4(p[0].y), (d2_point)D2_FIX4(p[1].x),
                 (d2_point)D2_FIX4(p[1].y), (d2_point)D2_FIX4(p[2].x), (d2_point)D2_FIX4(p[2].y), flags);
