...
void LV_ATTRIBUTE_FAST_MEM lv_draw_label(lv_layer_t *layer, const lv_draw_label_dsc_t *dsc, const lv_area_t *coords)
{
...
void LV_ATTRIBUTE_FAST_MEM lv_draw_character(lv_layer_t *layer, lv_draw_label_dsc_t *dsc, const lv_point_t *point,
                                      ...
                                      const lv_area_t *coords, lv_draw_glyph_cb_t cb)
{
    ...
    if ((dsc->flag & LV_TEXT_FLAG_EXPAND) == 0) {
        ...
        lv_point_t p;
        lv_text_get_size(&p, dsc->text, dsc->font, dsc->letter_space, dsc->line_space, LV_COORD_MAX, dsc->flag);
        w = p.x;
    }
    ...
    uint32_t line_end =
        line_start + lv_text_get_next_line(&dsc->text[line_start], font, dsc->letter_space, w, NULL, dsc->flag);
        ...
        while (i < line_end - line_start) {
            ...
            if (i >= line_end - line_start) {
                ...
                if (dsc->decor & LV_TEXT_DECOR_STRIKETHROUGH) {
                    ...
                    fill_area.x2 = pos.x + letter_w - 1;
                    fill_area.y1 =
                        pos.y + (font->line_height - font->base_line) * 2 / 3 + font->underline_thickness / 2;
                    fill_area.y2 = fill_area.y1 + underline_width - 1;
            ...
            if (sel_start != 0xFFFF && sel_end != 0xFFFF && logical_char_pos >= sel_start &&
    ...
    if (g.resolved_font) {
        ...
        if (LV_FONT_GLYPH_FORMAT_NONE < g.format && g.format < LV_FONT_GLYPH_FORMAT_IMAGE) {
            ...
            if (draw_buf == NULL) {
                ...
                draw_buf =
                    lv_draw_buf_create_ex(font_draw_buf_handlers, g.box_w, h, LV_COLOR_FORMAT_A8, LV_STRIDE_AUTO);
