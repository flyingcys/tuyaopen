...
void lv_draw_pxp_rotate(const void *src_buf, void *dest_buf, int32_t src_width, int32_t src_height, int32_t src_stride,
                        int32_t dest_stride, lv_display_rotation_t rotation, lv_color_format_t cf)
{
    lv_pxp_reset();
    ...
    pxp_as_buffer_config_t asBufferConfig = {
        .pixelFormat = pxp_get_as_px_format(cf), .bufferAddr = (uint32_t)src_buf, .pitchBytes = src_stride};
    ...
    /*Output buffer.*/
    pxp_output_buffer_config_t outputBufferConfig = {.pixelFormat = pxp_get_out_px_format(cf),
...
static int32_t _pxp_evaluate(lv_draw_unit_t *u, lv_draw_task_t *t)
{
    ...
    case LV_DRAW_TASK_TYPE_IMAGE: {
        ...
        if ((!_pxp_src_cf_supported(img_dsc->header.cf)) || (!pxp_buf_aligned(img_dsc->data, img_dsc->header.stride)))
            return 0;
