 ...
 *
 */
static void _vglite_draw_rect(const lv_area_t *coords, const lv_area_t *clip_area, const lv_draw_fill_dsc_t *dsc);
...
void lv_draw_vglite_fill(lv_draw_unit_t *draw_unit, const lv_draw_fill_dsc_t *dsc, const lv_area_t *coords)
{
...
static void _vglite_fill(const lv_area_t *dest_area, const lv_draw_fill_dsc_t *dsc)
{
    ...
    if (dsc->opa >= (lv_opa_t)LV_OPA_MAX) { /*Opaque fill*/
        vg_lite_rectangle_t rect = {.x = dest_area->x1,
                                    .y = dest_area->y1,
        ...
        int32_t path_data[] = {/*VG rectangular path*/
                               VLC_OP_MOVE,       dest_area->x1, dest_area->y1, VLC_OP_LINE,
                               dest_area->x2 + 1, dest_area->y1, VLC_OP_LINE,   dest_area->x2 + 1,
                               dest_area->y2 + 1, VLC_OP_LINE,   dest_area->x1, dest_area->y2 + 1,
                               VLC_OP_LINE,       dest_area->x1, dest_area->y1, VLC_OP_END};
        ...
        VGLITE_CHECK_ERROR(vg_lite_init_path(&path, VG_LITE_S32, VG_LITE_MEDIUM, sizeof(path_data), path_data,
                                             ...
                                             ((vg_lite_float_t)dest_area->y2) + 1.0f));
...
static void _vglite_draw_rect(const lv_area_t *coords, const lv_area_t *clip_area, const lv_draw_fill_dsc_t *dsc)
{
    ...
    if (has_gradient) {
        ...
        VGLITE_CHECK_ERROR(
            vg_lite_draw_gradient(vgbuf, &path, VG_LITE_FILL_EVEN_ODD, &matrix, &gradient, VG_LITE_BLEND_SRC_OVER));
    } else {
        VGLITE_CHECK_ERROR(vg_lite_draw(vgbuf, &path, VG_LITE_FILL_EVEN_ODD, &matrix, VG_LITE_BLEND_SRC_OVER, vgcol));
    }
