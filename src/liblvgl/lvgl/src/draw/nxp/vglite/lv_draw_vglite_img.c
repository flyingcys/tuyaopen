 ...
 *
 */
static void _vglite_draw_pattern(const lv_area_t *clip_area, const lv_area_t *coords, const lv_draw_image_dsc_t *dsc);
...
void lv_draw_vglite_img(lv_draw_unit_t *draw_unit, const lv_draw_image_dsc_t *dsc, const lv_area_t *coords)
{
    ...
    if (!has_transform)
        _vglite_blit_split(dest_buf, &blend_area, dest_stride, dest_cf, src_buf, &src_area, src_stride, src_cf, dsc);
#else
...
static void _vglite_blit(const lv_area_t *src_area, const lv_draw_image_dsc_t *dsc)
{
    ...
    vg_lite_rectangle_t rect = {.x = (vg_lite_int32_t)src_area->x1,
                               ...
                               const lv_draw_image_dsc_t *dsc)
{
    VGLITE_TRACE("Blit "
                 ...
                 "Size: ([%dx%d] -> [%dx%d]) | "
                 "Addr: (0x%x -> 0x%x)",
                 src_area->x1, src_area->y1, src_area->x2, src_area->y2, dest_area->x1, dest_area->y1, dest_area->x2,
                 dest_area->y2, lv_area_get_width(src_area), lv_area_get_height(src_area), lv_area_get_width(dest_area),
                 lv_area_get_height(dest_area), (uintptr_t)src_buf, (uintptr_t)dest_buf);
    ...
    if ((src_area->x2 < VGLITE_BLIT_SPLIT_THR) && (src_area->y2 < VGLITE_BLIT_SPLIT_THR)) {
        ...
        VGLITE_TRACE("Single "
                     ...
                     "Size: ([%dx%d] -> [%dx%d]) | "
                     "Addr: (0x%x -> 0x%x)",
                     src_area->x1, src_area->y1, src_area->x2, src_area->y2, dest_area->x1, dest_area->y1,
                     dest_area->x2, dest_area->y2, lv_area_get_width(src_area), lv_area_get_height(src_area),
                     lv_area_get_width(dest_area), lv_area_get_height(dest_area), (uintptr_t)src_buf,
                     (uintptr_t)dest_buf);
    ...
    uint8_t total_tiles_x = (src_area->x1 + width + VGLITE_BLIT_SPLIT_THR - 1) / VGLITE_BLIT_SPLIT_THR;
    uint8_t total_tiles_y = (src_area->y1 + height + VGLITE_BLIT_SPLIT_THR - 1) / VGLITE_BLIT_SPLIT_THR;
    ...
    for (uint8_t y = 0; y < total_tiles_y; y++) {
        ...
        for (uint8_t x = 0; x < total_tiles_x; x++) {
            ...
            VGLITE_TRACE("Tile [%d, %d] "
                         ...
                         "Size: ([%dx%d] -> [%dx%d]) | "
                         "Addr: (0x%x -> 0x%x)",
                         x, y, tile_src_area.x1, tile_src_area.y1, tile_src_area.x2, tile_src_area.y2,
...
static void _vglite_draw_pattern(const lv_area_t *clip_area, const lv_area_t *coords, const lv_draw_image_dsc_t *dsc)
{
    ...
    /* Draw Pattern */
    VGLITE_CHECK_ERROR(vg_lite_draw_pattern(dst_vgbuf, &path, VG_LITE_FILL_NON_ZERO, &path_matrix, src_vgbuf, &vgmatrix,
                                            vgblend, VG_LITE_PATTERN_REPEAT, 0, vgcol, filter));
}
