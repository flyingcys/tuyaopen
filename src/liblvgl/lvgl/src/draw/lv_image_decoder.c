...
lv_result_t lv_image_decoder_open(lv_image_decoder_dsc_t *dsc, const void *src, const lv_image_decoder_args_t *args)
{
    ...
    if (dsc->args.flush_cache && res == LV_RESULT_OK && dsc->decoded != NULL) {
        ...
        LV_LOG_INFO("Flushed D-cache: src %p (%s) (W%d x H%d, data: %p cf: %d)", src,
                    dsc->src_type == LV_IMAGE_SRC_FILE ? (const char *)src : "c-array", dsc->decoded->header.w,
                    dsc->decoded->header.h, (void *)dsc->decoded->data, dsc->decoded->header.cf);
    }
...
lv_draw_buf_t *lv_image_decoder_post_process(lv_image_decoder_dsc_t *dsc, lv_draw_buf_t *decoded)
{
    ...
    if (args->stride_align && decoded->header.cf != LV_COLOR_FORMAT_RGB565A8) {
        ...
        if (decoded->header.stride != stride_expect) {
            ...
            if (res != LV_RESULT_OK) {
                lv_draw_buf_t *aligned = lv_draw_buf_create_ex(image_cache_draw_buf_handlers, decoded->header.w,
                                                               decoded->header.h, decoded->header.cf, stride_expect);
                if (aligned == NULL) {
    ...
    if (args->premultiply && !LV_COLOR_FORMAT_IS_ALPHA_ONLY(decoded->header.cf) &&
