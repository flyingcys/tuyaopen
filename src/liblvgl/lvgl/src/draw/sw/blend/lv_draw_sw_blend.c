...
void lv_draw_sw_blend(lv_draw_unit_t *draw_unit, const lv_draw_sw_blend_dsc_t *blend_dsc)
{
    ...
    if (blend_dsc->src_buf == NULL) {
        ...
        if (fill_dsc.mask_buf) {
            fill_dsc.mask_stride =
                blend_dsc->mask_stride == 0 ? lv_area_get_width(blend_dsc->mask_area) : blend_dsc->mask_stride;
        ...
        if (image_dsc.mask_buf) {
            image_dsc.mask_buf = blend_dsc->mask_buf;
            image_dsc.mask_stride =
                blend_dsc->mask_stride ? blend_dsc->mask_stride : lv_area_get_width(blend_dsc->mask_area);
