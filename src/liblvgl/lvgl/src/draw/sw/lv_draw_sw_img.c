 ...
 *********************/
#define MAX_BUF_SIZE                                                                                                   \
    (uint32_t)(4 * lv_display_get_horizontal_resolution(lv_refr_get_disp_refreshing()) *                               \
               lv_color_format_get_size(lv_display_get_color_format(lv_refr_get_disp_refreshing())))
                          ...
                          const lv_area_t *img_coords, const lv_area_t *clipped_img_area)
{
    ...
    /* check whether it is possible to accelerate the operation in synchronous mode */
    else if (LV_RESULT_INVALID ==
             LV_DRAW_SW_IMAGE(transformed,      /* whether require transform */
                              cf,               /* image format */
        ...
        while (blend_area.y1 <= y_last) {
            ...
            if (transformed) {
                lv_draw_sw_transform(draw_unit, &relative_area, src_buf, src_w, src_h, img_stride, draw_dsc, sup, cf,
                                     tmp_buf);
                ...
                if (cf == LV_COLOR_FORMAT_L8) {
                    ...
                    const uint8_t *a_src_buf =
                        src_buf + stride_px * 2 * src_h + stride_px * relative_area.y1 + relative_area.x1;
