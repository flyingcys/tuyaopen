...
#define LV_DRAW_SW_RGB565_SWAP(__buf_ptr, __buf_size_px) lv_draw_sw_rgb565_swap_helium((__buf_ptr), (__buf_size_px))
#endif

#ifndef LV_DRAW_SW_IMAGE
#define LV_DRAW_SW_IMAGE(__transformed, __cf, __src_buf, __img_coords, __src_stride, __blend_area, __draw_unit,        \
                         __draw_dsc)                                                                                   \
    lv_draw_sw_image_helium((__transformed), (__cf), (uint8_t *)(__src_buf), (__img_coords), (__src_stride),           \
                            (__blend_area), (__draw_unit), (__draw_dsc))
#endif

#ifndef LV_DRAW_SW_RGB565_RECOLOR
#define LV_DRAW_SW_RGB565_RECOLOR(__src_buf, __blend_area, __color, __opa)                                             \
    lv_draw_sw_image_recolor_rgb565((__src_buf), &(__blend_area), (__color), (__opa))
#endif

#ifndef LV_DRAW_SW_RGB888_RECOLOR
#define LV_DRAW_SW_RGB888_RECOLOR(__src_buf, __blend_area, __color, __opa, __cf)                                       \
    lv_draw_sw_image_recolor_rgb888((__src_buf), &(__blend_area), (__color), (__opa), (__cf))
#endif
    ...
    do {                                                                                                               \
        ...
        if (draw_dsc->recolor_opa > LV_OPA_MIN) {                                                                      \
            if (LV_COLOR_FORMAT_RGB565 == des_cf) {                                                                    \
                rgb_tmp_buf = lv_malloc(src_w * src_h * sizeof(uint16_t));                                             \
                if (NULL == rgb_tmp_buf) {                                                                             \
                    LV_LOG_WARN("Failed to allocate memory for accelerating recolor, "                                 \
                                "use normal route instead.");                                                          \
                    break;                                                                                             \
                }                                                                                                      \
                lv_memcpy(rgb_tmp_buf, src_buf, src_w *src_h * sizeof(uint16_t));                                      \
                arm_2d_size_t copy_size = {                                                                            \
                    .iWidth = src_w,                                                                                   \
                    .iHeight = src_h,                                                                                  \
                };                                                                                                     \
                /* apply re-color */                                                                                   \
                __arm_2d_impl_rgb565_colour_filling_with_opacity((uint16_t *)rgb_tmp_buf, src_w, &copy_size,           \
                ...
                rgb_tmp_buf = lv_malloc(src_w * src_h * sizeof(uint32_t));                                             \
                if (NULL == rgb_tmp_buf) {                                                                             \
                    LV_LOG_WARN("Failed to allocate memory for accelerating recolor, "                                 \
                                "use normal route instead.");                                                          \
                    break;                                                                                             \
                }                                                                                                      \
                lv_memcpy(rgb_tmp_buf, src_buf, src_w *src_h * sizeof(uint32_t));                                      \
                arm_2d_size_t copy_size = {                                                                            \
                    .iWidth = src_w,                                                                                   \
                    .iHeight = src_h,                                                                                  \
                };                                                                                                     \
                /* apply re-color */                                                                                   \
                __arm_2d_impl_cccn888_colour_filling_with_opacity((uint32_t *)rgb_tmp_buf, src_w, &copy_size,          \
...
static inline lv_result_t lv_draw_sw_image_helium(bool is_transform, lv_color_format_t src_cf, const uint8_t *src_buf,
                                                  const lv_area_t *coords, int32_t src_stride,
                                                  const lv_area_t *des_area, lv_draw_unit_t *draw_unit,
                                                  const lv_draw_image_dsc_t *draw_dsc)
{
    ...
    do {
        ...
        if ((LV_COLOR_FORMAT_RGB565 == des_cf) &&
            !((LV_COLOR_FORMAT_RGB565 == src_cf) || (LV_COLOR_FORMAT_RGB565A8 == src_cf))) {
            break;
        }
...
#else
        if ((LV_COLOR_FORMAT_XRGB8888 == des_cf) || (LV_COLOR_FORMAT_RGB888 == des_cf) ||
        ...
        arm_2d_tile_generate_child(&target_tile_origin, &clip_region, &target_tile, false);
        ...
        if (LV_COLOR_FORMAT_A8 == src_cf) {
            ...
            if (LV_COLOR_FORMAT_RGB565 == des_cf) {
                ...
                arm_2d_rgb565_fill_colour_with_mask_opacity_and_transform(
                    &source_tile, &target_tile, NULL, source_center, ARM_2D_ANGLE((draw_dsc->rotation / 10.0f)),
                    draw_dsc->scale_x / 256.0f, lv_color_to_u16(draw_dsc->recolor), opa, &target_center);

            } else if (LV_COLOR_FORMAT_XRGB8888 == des_cf) {
                arm_2d_cccn888_fill_colour_with_mask_opacity_and_transform(
                    &source_tile, &target_tile, NULL, source_center, ARM_2D_ANGLE((draw_dsc->rotation / 10.0f)),
                    draw_dsc->scale_x / 256.0f, lv_color_to_int(draw_dsc->recolor), opa, &target_center);
            } else {
                break;
            }
            ...
            if (opa >= LV_OPA_MAX) {
                arm_2d_rgb565_tile_transform_with_src_mask(&source_tile, &mask_tile, &target_tile, NULL, source_center,
                                                           ARM_2D_ANGLE((draw_dsc->rotation / 10.0f)),
                                                           draw_dsc->scale_x / 256.0f, &target_center);
            } else {
                arm_2d_rgb565_tile_transform_with_src_mask_and_opacity(
                    &source_tile, &mask_tile, &target_tile, NULL, source_center,
                    ARM_2D_ANGLE((draw_dsc->rotation / 10.0f)), draw_dsc->scale_x / 256.0f, opa, &target_center);
            }
            ...
            if (opa >= LV_OPA_MAX) {
                ...
                arm_2d_rgb565_tile_transform_only(&source_tile, &target_tile, NULL, source_center,
                                                  ARM_2D_ANGLE((draw_dsc->rotation / 10.0f)),
                                                  draw_dsc->scale_x / 256.0f, &target_center);
#else

                arm_2dp_rgb565_tile_transform_only_prepare(NULL, &source_tile, source_center,
                                                           ARM_2D_ANGLE((draw_dsc->rotation / 10.0f)),
                                                           (float)(draw_dsc->scale_x / 256.0f));

                arm_2dp_tile_transform(NULL, &target_tile, NULL, &target_center);
#endif
            } else {
                arm_2d_rgb565_tile_transform_only_with_opacity(&source_tile, &target_tile, NULL, source_center,
                                                               ARM_2D_ANGLE((draw_dsc->rotation / 10.0f)),
                                                               draw_dsc->scale_x / 256.0f, opa, &target_center);
            }
...
static inline lv_result_t lv_draw_sw_image_recolor_rgb565(const uint8_t *src_buf, const lv_area_t *blend_area,
                                                          lv_color_t color, lv_opa_t opa)
{
    ...
    __arm_2d_impl_rgb565_colour_filling_with_opacity((uint16_t *)src_buf, src_w, &copy_size, lv_color_to_u16(color),
                                                     opa);

    return LV_RESULT_OK;
}

static inline lv_result_t lv_draw_sw_image_recolor_rgb888(const uint8_t *src_buf, const lv_area_t *blend_area,
                                                          lv_color_t color, lv_opa_t opa, lv_color_format_t src_cf)
{
    ...
    __arm_2d_impl_cccn888_colour_filling_with_opacity((uint32_t *)src_buf, src_w, &copy_size, lv_color_to_u32(color),
                                                      opa);
