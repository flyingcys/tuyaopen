...
void lv_draw_rect(lv_layer_t *layer, const lv_draw_rect_dsc_t *dsc, const lv_area_t *coords)
{
    ...
    if (dsc->shadow_width == 0 || dsc->shadow_opa <= LV_OPA_MIN ||
        (dsc->shadow_width == 1 && dsc->shadow_spread <= 0 && dsc->shadow_offset_x == 0 && dsc->shadow_offset_y == 0)) {
        has_shadow = false;
    } else {
        has_shadow = true;
    }
    ...
    if (has_bg_img) {
        ...
        if (res == LV_RESULT_OK) {
            if (src_type == LV_IMAGE_SRC_VARIABLE || src_type == LV_IMAGE_SRC_FILE) {
                ...
                lv_point_t s;
                lv_text_get_size(&s, dsc->bg_image_src, dsc->bg_image_symbol_font, 0, 0, LV_COORD_MAX,
                                 LV_TEXT_FLAG_NONE);
    ...
    if (has_outline) {
        ...
        outline_dsc->radius =
            dsc->radius == LV_RADIUS_CIRCLE ? LV_RADIUS_CIRCLE : dsc->radius + dsc->outline_width + dsc->outline_pad;
