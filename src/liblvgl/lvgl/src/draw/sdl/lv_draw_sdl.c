...
void lv_draw_sdl_init(void)
{
    ...
    draw_sdl_unit->texture_cache =
        lv_cache_create(&lv_cache_class_lru_rb_count, sizeof(cache_data_t), 128,
                        (lv_cache_ops_t){
...
static int32_t dispatch(lv_draw_unit_t *draw_unit, lv_layer_t *layer)
{
    ...
    if (layer != disp->layer_head && texture == NULL) {
        ...
        layer->user_data = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_TARGET, w, h);
    }
...
static bool draw_to_texture(lv_draw_sdl_unit_t *u, cache_data_t *cache_data)
{
    ...
    lv_draw_buf_init(dest_layer.draw_buf, texture_w, texture_h, LV_COLOR_FORMAT_ARGB8888, LV_STRIDE_AUTO,
                     sdl_render_buf, sizeof(sdl_render_buf));
...
static void execute_drawing(lv_draw_sdl_unit_t *u)
{
    ...
    if (t->type == LV_DRAW_TASK_TYPE_FILL) {
        ...
        if (fill_dsc->radius == 0 && fill_dsc->grad.dir == LV_GRAD_DIR_NONE) {
            ...
            SDL_SetRenderDrawColor(renderer, fill_dsc->color.red, fill_dsc->color.green, fill_dsc->color.blue,
                                   fill_dsc->opa);
            SDL_RenderSetClipRect(renderer, NULL);
