...
struct lv_layer_t {
     ...
     * Therefore `lv_draw_add_task()` always saves it in the new draw task to know the clip area when the draw task was
     * added. During drawing the draw units also sees the saved clip_area and should use it during drawing. During
     * drawing the layer's clip area shouldn't be used as it might be already changed for other draw tasks.
     */
