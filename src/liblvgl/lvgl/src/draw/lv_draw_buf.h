...
#define LV_DRAW_BUF_SIZE(w, h, cf)                                                                                     \
    (LV_DRAW_BUF_STRIDE(w, cf) * (h) + LV_DRAW_BUF_ALIGN + LV_COLOR_INDEXED_PALETTE_SIZE(cf) * sizeof(lv_color32_t))
...
#define LV_DRAW_BUF_INIT_STATIC(name)                                                                                  \
    do {                                                                                                               \
        ...
        lv_draw_buf_init(&name, header->w, header->h, (lv_color_format_t)header->cf, header->stride, buf_##name,       \
                         sizeof(buf_##name));                                                                          \
