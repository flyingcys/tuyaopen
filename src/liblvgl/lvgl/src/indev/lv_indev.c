...
static void indev_pointer_proc(lv_indev_t *i, lv_indev_data_t *data)
{
    ...
    if (i->cursor != NULL && (i->pointer.last_point.x != data->point.x || i->pointer.last_point.y != data->point.y)) {
    ...
    if (data->state == LV_INDEV_STATE_PRESSED && last_state == LV_INDEV_STATE_RELEASED) {
        ...
        if (data->key == LV_KEY_ENTER) {
            bool editable_or_scrollable =
                lv_obj_is_editable(indev_obj_act) || lv_obj_has_flag(indev_obj_act, LV_OBJ_FLAG_SCROLLABLE);
                ...
                if (editable_or_scrollable) {
                    ...
                    if (lv_group_get_obj_count(g) > 1) {
                        ...
                        lv_group_set_editing(g,
                                             lv_group_get_editing(g) ? false : true); /*Toggle edit mode on long press*/
...
static void indev_proc_press(lv_indev_t *indev)
{
    LV_LOG_INFO("pressed at x:%d y:%d", (int)indev->pointer.act_point.x, (int)indev->pointer.act_point.y);
    ...
    else if (indev->pointer.scroll_obj == NULL && lv_obj_has_flag(indev_obj_act, LV_OBJ_FLAG_PRESS_LOCK) == false) {
        indev_obj_act = pointer_search_obj(disp, &indev->pointer.act_point);
        new_obj_searched = true;
    }
...
static lv_result_t send_event(lv_event_code_t code, void *param)
{
    ...
    if (code == LV_EVENT_PRESSED || code == LV_EVENT_SHORT_CLICKED || code == LV_EVENT_CLICKED ||
        code == LV_EVENT_RELEASED || code == LV_EVENT_LONG_PRESSED || code == LV_EVENT_LONG_PRESSED_REPEAT ||
