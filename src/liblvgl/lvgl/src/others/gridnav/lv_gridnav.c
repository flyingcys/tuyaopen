...
static void gridnav_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_KEY) {
        ...
        if (dsc->focused_obj) {
            ...
            lv_obj_remove_state(dsc->focused_obj,
                                LV_STATE_PRESSED); /*Be sure the focuses obj is not stuck in pressed state*/
        ...
        lv_gridnav_remove(obj);
               ...
               code == LV_EVENT_LONG_PRESSED_REPEAT || code == LV_EVENT_CLICKED || code == LV_EVENT_RELEASED) {
        if (lv_group_get_focused(lv_obj_get_group(obj)) == obj) {
...
static lv_obj_t *find_chid(lv_obj_t *obj, lv_obj_t *start_child, find_mode_t mode)
{
    ...
    for (i = 0; i < child_cnt; i++) {
        ...
        if (guess == NULL || (y_err * y_err + x_err * x_err < y_err_guess * y_err_guess + x_err_guess * x_err_guess)) {
            guess = child;
            x_err_guess = x_err;
            y_err_guess = y_err;
        }
