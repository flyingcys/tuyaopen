...
static bool lv_text_get_snippet(const char *txt, const lv_font_t *font, int32_t letter_space, int32_t max_width,
                                lv_text_flag_t flag, int32_t *use_width, uint32_t *end_ofs);
...
int32_t lv_spangroup_get_expand_height(lv_obj_t *obj, int32_t width)
{
    ...
    while (cur_span) {
        ...
        while (1) {
            ...
            uint32_t next_ofs = 0;
            int32_t use_width = 0;
            bool isfill = lv_text_get_snippet(&cur_txt[cur_txt_ofs], snippet.font, snippet.letter_space, max_w,
                                              txt_flag, &use_width, &next_ofs);
            ...
            if (isfill && next_ofs > 0 && snippet_cnt > 0) {
                ...
                if (!(letter == '\0' || letter == '\n' || letter == '\r' || lv_text_is_break_char(letter) ||
                    ...
                    if (!(letter_next == '\0' || letter_next == '\n' || letter_next == '\r' ||
                          lv_text_is_break_char(letter_next))) {
                        break;
                    }
                }
            }
        ...
        for (i = 0; i < item_cnt; i++) {
            ...
            if (decor != LV_TEXT_DECOR_NONE) {
                ...
                if (decor & LV_TEXT_DECOR_UNDERLINE) {
                    int32_t y =
                        pos.y + pinfo->line_h - line_space - pinfo->font->base_line - pinfo->font->underline_position;
                    lv_point_precise_set(&line_dsc.p1, txt_pos.x, y);
                    lv_point_precise_set(&line_dsc.p2, pos.x, y);
                    lv_draw_line(layer, &line_dsc);
                }
