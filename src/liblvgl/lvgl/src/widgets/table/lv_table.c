...
static int32_t get_row_height(lv_obj_t *obj, uint32_t row_id, const lv_font_t *font, int32_t letter_space,
                              int32_t line_space, int32_t cell_left, int32_t cell_right, int32_t cell_top,
                              int32_t cell_bottom);
...
void lv_table_set_cell_value_fmt(lv_obj_t *obj, uint32_t row, uint32_t col, const char *fmt, ...)
{
    ...
    table->cell_data[cell] =
        lv_realloc(table->cell_data[cell], sizeof(lv_table_cell_t) + len + 1); /*+1: trailing '\0; */
    LV_ASSERT_MALLOC(table->cell_data[cell]);
    if (table->cell_data[cell] == NULL) {
        va_end(ap2);
        return;
    ...
    for (row = 0; row < table->row_cnt; row++) {
        ...
        for (col = 0; col < table->col_cnt; col++) {
            ...
            if (table->cell_data[cell]) {
                ...
                lv_text_get_size(&txt_size, table->cell_data[cell]->txt, label_dsc_def.font, label_dsc_act.letter_space,
                                 label_dsc_act.line_space, lv_area_get_width(&txt_area), txt_flags);
...
static void refr_size_form_row(lv_obj_t *obj, uint32_t start_row)
{
    ...
    for (i = start_row; i < table->row_cnt; i++) {
        int32_t calculated_height = get_row_height(obj, i, font, letter_space, line_space, cell_pad_left,
...
static void refr_cell_size(lv_obj_t *obj, uint32_t row, uint32_t col)
{
    ...
    lv_table_t *table = (lv_table_t *)obj;
    int32_t calculated_height = get_row_height(obj, row, font, letter_space, line_space, cell_pad_left, cell_pad_right,
                                               cell_pad_top, cell_pad_bottom);
                              ...
                              int32_t cell_bottom)
{
    ...
    for (cell = row_start, col = 0; cell < row_start + table->col_cnt; cell++, col++) {
        ...
        if (ctrl & LV_TABLE_CELL_CTRL_TEXT_CROP) {
            h_max = LV_MAX(lv_font_get_line_height(font) + cell_top + cell_bottom, h_max);
        }
        /*Else we have to calculate the height of the cell text*/
            ...
            lv_text_get_size(&txt_size, table->cell_data[cell]->txt, font, letter_space, line_space, txt_w,
                             LV_TEXT_FLAG_NONE);
