...
void lv_roller_set_options(lv_obj_t *obj, const char *options, lv_roller_mode_t mode)
{
    ...
    if (mode == LV_ROLLER_MODE_NORMAL) {
        ...
        int32_t normal_h =
            roller->option_cnt * (lv_font_get_line_height(font) + lv_obj_get_style_text_letter_space(obj, 0));
    ...
    /*Post draw when the children are drawn*/
        ...
        if (area_ok) {
            ...
            int32_t label_y_prop =
                (label->coords.y1 + normal_label_font->line_height / 2) - (roller_h / 2 + obj->coords.y1);
