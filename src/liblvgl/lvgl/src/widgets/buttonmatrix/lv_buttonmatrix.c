    ...
    if (code == LV_EVENT_STYLE_CHANGED) {
            ...
            if (indev_type == LV_INDEV_TYPE_KEYPAD || (indev_type == LV_INDEV_TYPE_ENCODER && editing)) {
                ...
                if (btnm->one_check) {
                    while (b < btnm->btn_cnt &&
                           (button_is_hidden(btnm->ctrl_bits[b]) || button_is_inactive(btnm->ctrl_bits[b]) ||
        ...
        if (c == LV_KEY_RIGHT) {
            ...
            if (btnm->btn_id_sel == LV_BUTTONMATRIX_BUTTON_NONE) {
                ...
                uint32_t area_below;
                int32_t pr_center = btnm->button_areas[btnm->btn_id_sel].x1 +
...
#endif
        lv_point_t txt_size;
        lv_text_get_size(&txt_size, txt, font, letter_space, line_space, lv_area_get_width(&area_obj),
