...
static void month_event_cb(lv_event_t *e)
{
    ...
    LV_ASSERT_FORMAT_MSG(newd.year >= 0 && newd.month >= 1 && newd.month <= 12, "Invalid date: %d-%d", newd.year,
                         newd.month);
...
static void value_changed_event_cb(lv_event_t *e)
{
    ...
    LV_ASSERT_FORMAT_MSG(date->year >= 0 && date->month >= 1 && date->month <= 12, "Invalid date: %d-%d", date->year,
                         date->month);
