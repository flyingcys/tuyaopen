...
lv_result_t lv_calendar_get_pressed_date(const lv_obj_t *obj, lv_calendar_date_t *date)
{
    ...
    const char *txt =
        lv_buttonmatrix_get_button_text(calendar->btnm, lv_buttonmatrix_get_selected_button(calendar->btnm));
...
static void lv_calendar_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
    ...
    lv_buttonmatrix_set_button_ctrl_all(calendar->btnm,
                                        LV_BUTTONMATRIX_CTRL_CLICK_TRIG | LV_BUTTONMATRIX_CTRL_NO_REPEAT);
...
static void highlight_update(lv_obj_t *obj)
{
    ...
    if (calendar->showed_date.year == calendar->today.year && calendar->showed_date.month == calendar->today.month) {
        lv_buttonmatrix_set_button_ctrl(calendar->btnm, calendar->today.day - 1 + day_first + 7,
...
static void chinese_calendar_set_day_name(lv_obj_t *obj, uint8_t index, uint8_t day, lv_calendar_date_t *gregorian_time)
{
    lv_calendar_t *calendar = (lv_calendar_t *)obj;
    const char *day_name = lv_calendar_get_day_name(gregorian_time);
    if (day_name != NULL)
        lv_snprintf(calendar->nums[index], sizeof(calendar->nums[0]), "%d\n%s", day, day_name);
    else
