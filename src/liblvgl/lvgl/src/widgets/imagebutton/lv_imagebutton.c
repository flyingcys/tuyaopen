...
void lv_imagebutton_set_state(lv_obj_t *obj, lv_imagebutton_state_t state)
{
    ...
    if (state == LV_IMAGEBUTTON_STATE_PRESSED || state == LV_IMAGEBUTTON_STATE_CHECKED_PRESSED)
        obj_state |= LV_STATE_PRESSED;
    if (state == LV_IMAGEBUTTON_STATE_DISABLED || state == LV_IMAGEBUTTON_STATE_CHECKED_DISABLED)
...
static void lv_imagebutton_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
    ...
    if (code == LV_EVENT_PRESSED || code == LV_EVENT_RELEASED || code == LV_EVENT_PRESS_LOST) {
        ...
        if (imagebutton->src_left[state].img_src == NULL && imagebutton->src_mid[state].img_src != NULL &&
