...
void lv_label_get_letter_pos(const lv_obj_t *obj, uint32_t char_id, lv_point_t *pos)
{
    ...
    if (new_line_start == byte_id) {
        ...
        bool is_rtl;
        uint32_t visual_char_pos = lv_bidi_get_visual_pos(&txt[line_start], &mutable_bidi_txt,
                                                          new_line_start - line_start, base_dir, line_char_id, &is_rtl);
...
uint32_t lv_label_get_letter_on(const lv_obj_t *obj, lv_point_t *pos_in, bool bidi)
{
    ...
    if (bidi) {
        ...
        if (new_line_start > 0 && txt[new_line_start - 1] == '\0' && txt_len > 0)
            txt_len--;
        lv_bidi_process_paragraph(txt + line_start, bidi_txt, txt_len, lv_obj_get_style_base_dir(obj, LV_PART_MAIN),
                                  NULL, 0);
    } else
#endif
    {
...
static void draw_main(lv_event_t *e)
{
    ...
    if ((label->long_mode == LV_LABEL_LONG_SCROLL || label->long_mode == LV_LABEL_LONG_SCROLL_CIRCULAR) &&
        ...
        lv_point_t size;
        lv_text_get_size(&size, label->text, label_draw_dsc.font, label_draw_dsc.letter_space,
...
static void lv_label_refr_text(lv_obj_t *obj)
{
    ...
    /*In roll inf. mode keep the size but start offset animations*/
        ...
        if (size.y <= lv_area_get_height(&txt_coords)) { /*No dots are required, the text is short enough*/
            ...
            label->dot_end = LV_LABEL_DOT_END_INV;
        } else if (lv_text_get_encoded_length(label->text) <=
            ...
            y_overed = p.y % (lv_font_get_line_height(font) + line_space); /*Round down to the last line*/
...
static lv_text_flag_t get_label_flags(lv_label_t *label)
{
    ...
    if (lv_obj_get_style_width(obj, LV_PART_MAIN) == LV_SIZE_CONTENT &&
        lv_obj_get_style_max_width(obj, LV_PART_MAIN) == LV_COORD_MAX && !obj->w_layout) {
