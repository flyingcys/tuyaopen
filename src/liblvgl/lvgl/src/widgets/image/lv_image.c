...
#if LV_USE_LOG
        char buf[24];
        LV_LOG_WARN("failed to get image info: %s", src_type == LV_IMAGE_SRC_FILE
                                                        ? (const char *)src
                                                        : (lv_snprintf(buf, sizeof(buf), "%p", src), buf));
#endif /*LV_USE_LOG*/
        return;
    }
...
static void draw_image(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_COVER_CHECK) {
        ...
        if (img->scale_x == LV_SCALE_NONE && img->scale_y == LV_SCALE_NONE) {
            ...
            lv_point_t pivot_px;
            lv_image_get_pivot(obj, &pivot_px);
            lv_image_buf_get_transformed_area(&a, lv_obj_get_width(obj), lv_obj_get_height(obj), 0, img->scale_x,
                                              img->scale_y, &pivot_px);
            a.x1 += obj->coords.x1;
            a.y1 += obj->coords.y1;
            a.x2 += obj->coords.x1;
            a.y2 += obj->coords.y1;
        ...
        if (img->src_type == LV_IMAGE_SRC_FILE || img->src_type == LV_IMAGE_SRC_VARIABLE) {
            ...
            lv_area_set(&draw_dsc.image_area, obj->coords.x1, obj->coords.y1, obj->coords.x1 + img->w - 1,
