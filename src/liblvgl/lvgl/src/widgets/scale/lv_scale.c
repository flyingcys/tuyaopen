...
static void scale_get_tick_points(lv_obj_t *obj, const uint32_t tick_idx, bool is_major_tick, lv_point_t *tick_point_a,
...
static void scale_store_main_line_tick_width_compensation(lv_obj_t *obj, const uint32_t tick_idx,
                                                          const bool is_major_tick, const int32_t major_tick_width,
                                                          const int32_t minor_tick_width);
static void scale_store_section_line_tick_width_compensation(lv_obj_t *obj, const bool is_major_tick,
                                                             ...
                                                             const int32_t tick_value, const uint8_t tick_idx,
                                                             lv_point_t *tick_point_a);
static void scale_build_custom_label_text(lv_obj_t *obj, lv_draw_label_dsc_t *label_dsc, const uint16_t major_tick_idx);
...
void lv_scale_set_line_needle_value(lv_obj_t *obj, lv_obj_t *needle_line, int32_t needle_length, int32_t value)
{
    ...
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_scale_t *scale = (lv_scale_t *)obj;
    if ((scale->mode != LV_SCALE_MODE_ROUND_INNER) && (scale->mode != LV_SCALE_MODE_ROUND_OUTER)) {
        return;
    }
...
static void scale_calculate_main_compensation(lv_obj_t *obj)
{
    ...
    for (tick_idx = 0; tick_idx < total_tick_count; tick_idx++) {
        ...
        scale_store_main_line_tick_width_compensation(obj, tick_idx, is_major_tick, major_tick_dsc.width,
                                                      minor_tick_dsc.width);
...
static void scale_draw_main(lv_obj_t *obj, lv_event_t *event)
{
    ...
    if ((LV_SCALE_MODE_VERTICAL_LEFT == scale->mode || LV_SCALE_MODE_VERTICAL_RIGHT == scale->mode) ||
        ...
        LV_LL_READ_BACK(&scale->section_ll, section)
        {
            ...
            /* TODO: Add compensation for the width of the first and last tick over the arc */
            const int32_t section_start_angle = lv_map(section->minor_range, scale->range_min, scale->range_max,
                                                       scale->rotation, scale->rotation + scale->angle_range);
            const int32_t section_end_angle = lv_map(section->major_range, scale->range_min, scale->range_max,
                                                     scale->rotation, scale->rotation + scale->angle_range);
                                  ...
                                  lv_point_t *tick_point_b)
{
    ...
    if ((LV_SCALE_MODE_VERTICAL_LEFT == scale->mode || LV_SCALE_MODE_VERTICAL_RIGHT == scale->mode) ||
        ...
        if (LV_SCALE_MODE_VERTICAL_LEFT == scale->mode || LV_SCALE_MODE_VERTICAL_RIGHT == scale->mode) {
            /* Vertical position starts at y2 of the scale main line, we start at y2 because the ticks are drawn from
             * bottom to top */
            ...
            /* Otherwise adjust the tick position depending of its index and number of ticks on the scale */
                ...
                const int32_t scale_total_height =
                    lv_obj_get_height(obj) - (pad_top + pad_bottom + tick_pad_top + tick_pad_bottom);
            ...
            /* Otherwise adjust the tick position depending of its index and number of ticks on the scale */
                ...
                const int32_t scale_total_width =
                    lv_obj_get_width(obj) - (pad_right + pad_left + tick_pad_right + tick_pad_left);
                                   ...
                                   lv_area_t *label_coords)
{
    ...
    lv_text_get_size(&label_size, label_dsc->text, label_dsc->font, label_dsc->letter_space, label_dsc->line_space,
