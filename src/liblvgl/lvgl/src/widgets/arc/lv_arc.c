...
static void lv_arc_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
    ...
    if (code == LV_EVENT_PRESSING) {
        ...
        const uint32_t circumference =
            (uint32_t)((2U * r * 314U) / 100U); /* Equivalent to: 2r * 3.14, avoiding floats */
        ...
        int32_t new_value =
            lv_map((int32_t)angle, (int32_t)arc->bg_angle_start, (int32_t)bg_end, arc->min_value, arc->max_value);
...
static void value_update(lv_obj_t *obj)
{
    ...
    case LV_ARC_MODE_SYMMETRICAL:
        ...
        if (arc->value < range_midpoint) {
            angle =
                lv_map(arc->value, arc->min_value, range_midpoint, (int32_t)arc->bg_angle_start, (int32_t)bg_midpoint);
