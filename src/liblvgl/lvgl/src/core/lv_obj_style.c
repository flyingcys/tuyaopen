...
void lv_obj_add_style(lv_obj_t *obj, const lv_style_t *style, lv_style_selector_t selector)
{
    ...
    if (lv_style_is_const(style)) {
        ...
        lv_style_prop_t *props =
            (lv_style_prop_t *)style->values_and_props + style->prop_cnt * sizeof(lv_style_value_t);
                          ...
                          lv_style_selector_t selector)
{
    ...
    for (i = 0; i < obj->style_cnt; i++) {
        ...
        if ((state != LV_STATE_ANY && state_act != state) || (part != LV_PART_ANY && part_act != part) ||
...
void lv_obj_refresh_style(lv_obj_t *obj, lv_style_selector_t selector, lv_style_prop_t prop)
{
    ...
    if (is_layout_refr) {
        if (part == LV_PART_ANY || part == LV_PART_MAIN || lv_obj_get_style_height(obj, 0) == LV_SIZE_CONTENT ||
    ...
    for (i = 0; i < obj->style_cnt; i++) {
        ...
        default:
            if (v == 0)
                ...
                value_final.num =
                    tr->start_value.num + ((int32_t)((int32_t)(tr->end_value.num - tr->start_value.num) * v) >> 8);
            break;
        }
