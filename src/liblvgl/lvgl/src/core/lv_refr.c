...
{
    lv_draw_buf_t *ret = lv_draw_buf_reshape(layer->draw_buf, layer->color_format, lv_area_get_width(&layer->buf_area),
                                             lv_area_get_height(&layer->buf_area), stride);
    LV_UNUSED(ret);
    LV_ASSERT_NULL(ret);
}
...
static void refr_obj_matrix(lv_layer_t *layer, lv_obj_t *obj)
{
    ...
    if (scale_x != LV_SCALE_NONE || scale_y != LV_SCALE_NONE) {
        lv_matrix_scale(&obj_matrix, (float)scale_x / LV_SCALE_NONE, (float)scale_y / LV_SCALE_NONE);
    }
...
static void refr_obj(lv_layer_t *layer, lv_obj_t *obj)
{
    ...
    if (layer_type == LV_LAYER_TYPE_NONE) {
        ...
        while (layer_area_act.y2 < layer_area_full.y2) {
            ...
            lv_layer_t *new_layer = lv_draw_layer_create(
                layer, area_need_alpha ? LV_COLOR_FORMAT_ARGB8888 : LV_COLOR_FORMAT_NATIVE, &layer_area_act);
...
{
    LV_PROFILER_BEGIN;
    LV_TRACE_REFR("Calling flush_cb on (%d;%d)(%d;%d) area with %p image pointer", (int)area->x1, (int)area->y1,
                  (int)area->x2, (int)area->y2, (void *)px_map);

    lv_area_t offset_area = {.x1 = area->x1 + disp->offset_x,
