 ...
 *  GLOBAL VARIABLES
 **********************/
const lv_cache_class_t lv_cache_class_lru_rb_count = {.alloc_cb = alloc_cb,
                                                      .init_cb = init_cnt_cb,
...
const lv_cache_class_t lv_cache_class_lru_rb_size = {.alloc_cb = alloc_cb,
...
static bool init_cnt_cb(lv_cache_t *cache)
{
    ...
    if (!lv_rb_init(&lru->rb, lru->cache.ops.compare_cb,
                    lv_cache_entry_get_size(lru->cache.node_size) + sizeof(void *))) {
        return false;
    }
...
static void drop_all_cb(lv_cache_t *cache, void *user_data)
{
    ...
    LV_LL_READ(&lru->ll, node)
    {
        ...
        if (lv_cache_entry_get_ref(entry) == 0) {
            ...
            LV_LOG_WARN("entry (%p) is still referenced (%" LV_PRId32 ")", (void *)entry,
                                                   ...
                                                   void *user_data)
{
    ...
    return cache->size + reserved_size + data_size > lru->cache.max_size ? LV_CACHE_RESERVE_COND_NEED_VICTIM
