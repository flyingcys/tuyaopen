 ...
 * This function only cares about how to add the entry, it doesn't check if the entry already exists and doesn't care
 * about is it a victim or not.
...
/**
 * The cache remove function, used by the cache class to remove a cache entry from the cache but doesn't free the
 * memory.. This function only cares about how to remove the entry, it doesn't care about is it a victim or not.
 */
...
/**
 * The cache reserve condition function, used by the cache class to check if a new entry can be added to the cache
 * without exceeding its maximum size. See lv_cache_reserve_cond_res_t for the possible results.
 */
