...
lv_fs_res_t lv_fs_open(lv_fs_file_t *file_p, const char *path, lv_fs_mode_t mode)
{
    ...
    /* For memory-mapped files we set the file handle to our file descriptor so that we can access the cache from the
     * file operations */
    ...
    if (start <= file_position && file_position <= end) {
        ...
        if (btr <= buffer_remaining_length) {
            ...
            *br = btr;
            ...
            file_p->drv->seek_cb(file_p->drv, file_p->file_d, file_p->cache->end + 1, LV_FS_SEEK_SET);

            uint32_t bytes_read_to_buffer = 0;
            if (btr - buffer_remaining_length > buffer_size) {
                /*If remaining data chuck is bigger than buffer size, then do not use cache, instead read it directly
                 * from FS*/
        ...
        file_p->drv->seek_cb(file_p->drv, file_p->file_d, file_p->cache->file_position, LV_FS_SEEK_SET);
...
static lv_fs_res_t lv_fs_seek_cached(lv_fs_file_t *file_p, uint32_t pos, lv_fs_whence_t whence)
{
    ...
    case LV_FS_SEEK_END: {
        /*Because we don't know the file size, we do a little trick: do a FS seek, then get the new file position from
         * FS*/
        res = file_p->drv->seek_cb(file_p->drv, file_p->file_d, pos, whence);
        if (res == LV_FS_RES_OK) {
