...
void lv_text_ap_proc(const char *txt, char *txt_out)
{
    ...
    while (i < txt_length) {
        ...
        if (lv_text_is_arabic_vowel(ch_enc[i])) { // Current character is a vowel
            ...
            idx_next =
                lv_ap_get_char_index(ch_enc[i + 2]); // Skip the vowel character to join with the character after it
        }
        ...
        uint8_t conjunction_to_previous = (i == 0 || idx_previous == LV_UNDEF_ARABIC_PERSIAN_CHARS)
                                              ? 0
        ...
        if (conjunction_to_previous && conjunction_to_next)
            ...
            ch_fin[j] =
                ap_chars_map[index_current].char_end_form + ap_chars_map[index_current].char_beginning_form_offset;
        else if (conjunction_to_previous && !conjunction_to_next)
            ch_fin[j] = ap_chars_map[index_current].char_end_form;
        else
            ch_fin[j] =
                ap_chars_map[index_current].char_end_form + ap_chars_map[index_current].char_isolated_form_offset;
