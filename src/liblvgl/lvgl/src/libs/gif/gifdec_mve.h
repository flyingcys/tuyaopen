...
#define GIFDEC_FILL_BG(dst, w, h, stride, color, opa) _gifdec_fill_bg_mve(dst, w, h, stride, color, opa)
                                       ...
                                       uint8_t opa)
{
    ...
    __asm volatile(".p2align 2                                             \n"
                   ...
                   "bne                 3b                                 \n"
                   : [dst] "+r"(dst), [h] "+r"(h)
                   : [src] "r"(color_32), [w] "r"(w), [iTargetStride] "r"(stride * sizeof(uint32_t))
                   : "r0", "q0", "memory", "r14", "cc");
}
                                            ...
                                            uint8_t *pattern, uint16_t tindex)
{
    ...
    __asm volatile("vmov.u16       q3, #255                                \n"
                   "vshl.u16       q3, q3, #8                              \n" /* left shift 8 for a*/
                   ...
                   : [pattern] "r"(pattern), [w] "r"(w), [stride] "r"(stride), [tindex] "r"(tindex)
