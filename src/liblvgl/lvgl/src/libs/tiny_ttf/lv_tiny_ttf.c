    ...
    if (entry == NULL) {
        if (!dsc->cache_size) { /* no cache, do everything directly */
            ...
            *dsc_out = search_key.glyph_dsc;
            ...
            /*Kerning correction*/
            if (font->kerning == LV_FONT_KERNING_NORMAL && unicode_letter_next != 0) {
                int g2 = stbtt_FindGlyphIndex(&dsc->info,
                                              (int)unicode_letter_next); /* not using cache, only do glyph id lookup */
                if (g2) {
...
static void lv_tiny_ttf_cache_create(ttf_font_desc_t *dsc)
{
    /*Init cache*/
    dsc->glyph_cache =
        lv_cache_create(&lv_cache_class_lru_rb_count, sizeof(tiny_ttf_glyph_cache_data_t), dsc->cache_size,
                        (lv_cache_ops_t){.compare_cb = (lv_cache_compare_cb_t)tiny_ttf_glyph_cache_compare_cb,
...
#if LV_TINY_TTF_FILE_SUPPORT != 0
lv_font_t *lv_tiny_ttf_create_file_ex(const char *path, int32_t font_size, lv_font_kerning_t kerning, size_t cache_size)
{
...
lv_font_t *lv_tiny_ttf_create_data_ex(const void *data, size_t data_size, int32_t font_size, lv_font_kerning_t kerning,
                                      size_t cache_size)
{
...
static bool tiny_ttf_glyph_cache_create_cb(tiny_ttf_glyph_cache_data_t *node, void *user_data)
{
    ...
    if (dsc->kerning != LV_FONT_KERNING_NORMAL) { /* calculate default advance */
        ...
        dsc_out->adv_w =
            (uint16_t)floor(((float)advw * dsc->scale) + 0.5f); /*Horizontal space required by the glyph in [px]*/
        ;
    }
