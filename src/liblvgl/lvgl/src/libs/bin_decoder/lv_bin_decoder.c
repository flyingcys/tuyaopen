...
lv_result_t lv_bin_decoder_open(lv_image_decoder_t *decoder, lv_image_decoder_dsc_t *dsc)
{
    ...
    if (dsc->src_type == LV_IMAGE_SRC_FILE) {
...
#if LV_BIN_DECODER_RAM_LOAD
        else if (cf == LV_COLOR_FORMAT_ARGB8888 || cf == LV_COLOR_FORMAT_XRGB8888 || cf == LV_COLOR_FORMAT_RGB888 ||
                 cf == LV_COLOR_FORMAT_RGB565 || cf == LV_COLOR_FORMAT_RGB565A8 || cf == LV_COLOR_FORMAT_ARGB8565) {
        ...
        if (dsc->header.flags & LV_IMAGE_FLAGS_COMPRESSED) {
            ...
            /*Alpha only image will need decoder data to store pointer to decoded image, to free it when decoder
             * closes*/
            decoder_data_t *decoder_data = get_decoder_data(dsc);
            if (decoder_data == NULL) {
                return LV_RESULT_INVALID;
                                    ...
                                    const lv_area_t *full_area, lv_area_t *decoded_area)
{
    ...
    bool supported = LV_COLOR_FORMAT_IS_INDEXED(cf) || cf == LV_COLOR_FORMAT_ARGB8888 ||
                     cf == LV_COLOR_FORMAT_XRGB8888 || cf == LV_COLOR_FORMAT_RGB888 || cf == LV_COLOR_FORMAT_RGB565 ||
                     cf == LV_COLOR_FORMAT_ARGB8565 || cf == LV_COLOR_FORMAT_RGB565A8;
    if (!supported) {
...
static lv_result_t decode_indexed(lv_image_decoder_t *decoder, lv_image_decoder_dsc_t *dsc)
{
    ...
    if (is_compressed) {
...
#if LV_BIN_DECODER_RAM_LOAD
        draw_buf_indexed =
            lv_draw_buf_create_ex(image_cache_draw_buf_handlers, dsc->header.w, dsc->header.h, cf, dsc->header.stride);
        ...
        uint32_t data_len = 0;
        if (lv_fs_seek(f, 0, LV_FS_SEEK_END) != LV_FS_RES_OK || lv_fs_tell(f, &data_len) != LV_FS_RES_OK) {
    ...
    lv_draw_buf_t *decoded =
        lv_draw_buf_create_ex(image_cache_draw_buf_handlers, dsc->header.w, dsc->header.h, LV_COLOR_FORMAT_ARGB8888, 0);
    if (decoded == NULL) {
...
static lv_result_t decode_compressed(lv_image_decoder_t *decoder, lv_image_decoder_dsc_t *dsc)
{
    ...
    if (dsc->src_type == LV_IMAGE_SRC_FILE) {
        ...
        if (lv_fs_seek(f, 0, LV_FS_SEEK_END) != LV_FS_RES_OK || lv_fs_tell(f, &compressed_len) != LV_FS_RES_OK) {
...
static lv_result_t decompress_image(lv_image_decoder_dsc_t *dsc, const lv_image_compressed_t *compressed)
{
    ...
    lv_draw_buf_t *decompressed = lv_draw_buf_create_ex(image_cache_draw_buf_handlers, dsc->header.w, dsc->header.h,
                                                        dsc->header.cf, dsc->header.stride);
