 ...
 *      MACROS
 **********************/
#define TRANS_32_VALUE(big_endian, data)                                                                               \
    big_endian ? ((*(data) << 24) | (*((data) + 1) << 16) | (*((data) + 2) << 8) | *((data) + 3))                      \
               : (*(data) | (*((data) + 1) << 8) | (*((data) + 2) << 16) | (*((data) + 3) << 24))
#define TRANS_16_VALUE(big_endian, data)                                                                               \
    big_endian ? ((*(data) << 8) | *((data) + 1)) : (*(data) | (*((data) + 1) << 8))
    ...
    while (marker != NULL) {
        if (marker->marker == JPEG_APP0 + 1) {
            ...
            if (TRANS_32_VALUE(true, app1_data) == JPEG_EXIF) {
                ...
                unsigned char *ifd = 0;
                do {
                    /* ifd start: 4bytes(Exif) + 2bytes(0x00) + offset value(2bytes(align) + 2bytes(tag mark) +
                     * 4bytes(offset size)) */
                    ...
                    for (int i = 0; i < num_entries; i++) {
                        ...
                        if (tag == 0x0112) {
                            /* ifd entry: 12bytes = 2bytes(tag number) + 2bytes(kind of data) + 4bytes(number of
                             * components) + 4bytes(data) orientation kind(0x03) of data is unsigned short */
