 ...
 *
 * It sets up the target buffer, which can be drawn on the screen. It stores the Tvg_Paint objects (Shape, Scene,
 * Picture).
 */
...
typedef enum {
    ...
    TVG_RESULT_INVALID_ARGUMENT, ///< The value returned in the event of a problem with the arguments given to the API -
                                 ///< e.g. empty paths or null pointers.
    TVG_RESULT_INSUFFICIENT_CONDITION, ///< The value returned in case the request cannot be processed - e.g. asking for
    ...
    TVG_RESULT_MEMORY_CORRUPTION, ///< The value returned in the event of bad memory handling - e.g. failing in pointer
                                  ///< releasing or casting
...
typedef enum {
    ...
    TVG_COMPOSITE_METHOD_CLIP_PATH,  ///< The intersection of the source and the target is determined and only the
                                     ///< resulting pixels from the source are rendered.
    TVG_COMPOSITE_METHOD_ALPHA_MASK, ///< The pixels of the source and the target are alpha blended. As a result, only
                                     ///< the part of the source, which intersects with the target is visible.
    TVG_COMPOSITE_METHOD_INVERSE_ALPHA_MASK, ///< The pixels of the source and the complement to the target's pixels are
                                             ///< alpha blended. As a result, only the part of the source which is not
                                             ///< covered by the target is visible.
    TVG_COMPOSITE_METHOD_LUMA_MASK, ///< The source pixels are converted to grayscale (luma value) and alpha blended
                                    ///< with the target. As a result, only the part of the source which intersects with
                                    ///< the target is visible. \since 0.9
    TVG_COMPOSITE_METHOD_INVERSE_LUMA_MASK ///< The source pixels are converted to grayscale (luma value) and complement
                                           ///< to the target's pixels are alpha blended. As a result, only the part of
                                           ///< the source which is not covered by the target is visible. \Experimental
                                           ///< API
...
typedef enum {
    ...
    TVG_BLEND_METHOD_SCREEN, ///< The values of the pixels in the two layers are inverted, multiplied, and then inverted
                             ///< again. (S + D) - (S * D)
    TVG_BLEND_METHOD_MULTIPLY, ///< Takes the RGB channel values from 0 to 255 of each pixel in the top layer and
                               ///< multiples them with the values for the corresponding pixel from the bottom layer. (S
                               ///< * D)
    TVG_BLEND_METHOD_OVERLAY,  ///< Combines Multiply and Screen blend modes. (2 * S * D) if (2 * D < Da), otherwise (Sa
                               ///< * Da) - 2 * (Da - S) * (Sa - D)
    TVG_BLEND_METHOD_DIFFERENCE, ///< Subtracts the bottom layer from the top layer or the other way around, to always
                                 ///< get a non-negative value. (S - D) if (S > D), otherwise (D - S)
    TVG_BLEND_METHOD_EXCLUSION, ///< The result is twice the product of the top and bottom layers, subtracted from their
                                ///< sum. s + d - (2 * s * d)
    TVG_BLEND_METHOD_SRCOVER,   ///< Replace the bottom layer with the top layer.
    TVG_BLEND_METHOD_DARKEN,    ///< Creates a pixel that retains the smallest components of the top and bottom layer
                                ///< pixels. min(S, D)
    ...
    TVG_BLEND_METHOD_COLORBURN,  ///< Divides the inverted bottom layer by the top layer, and then inverts the result.
                                 ///< 255 - (255 - D) / S
    TVG_BLEND_METHOD_HARDLIGHT,  ///< The same as Overlay but with the color roles reversed. (2 * S * D) if (S < Sa),
                                 ///< otherwise (Sa * Da) - 2 * (Da - S) * (Sa - D)
    TVG_BLEND_METHOD_SOFTLIGHT   ///< The same as Overlay but with applying pure black or white does not result in pure
                                 ///< black or white. (1 - 2 * S) * (D ^ 2) + (2 * S * D)
} Tvg_Blend_Method;
 ...
 */
typedef enum {
    TVG_PATH_COMMAND_CLOSE = 0, ///< Ends the current sub-path and connects it with its initial point - corresponds to Z
                                ///< command in the svg path commands.
    TVG_PATH_COMMAND_MOVE_TO,   ///< Sets a new initial point of the sub-path and a new current point - corresponds to M
                                ///< command in the svg path commands.
    TVG_PATH_COMMAND_LINE_TO,   ///< Draws a line from the current point to the given point and sets a new value of the
                                ///< current point - corresponds to L command in the svg path commands.
    TVG_PATH_COMMAND_CUBIC_TO ///< Draws a cubic Bezier curve from the current point to the given point using two given
                              ///< control points and sets a new value of the current point - corresponds to C command
                              ///< in the svg path commands.
} Tvg_Path_Command;
...
typedef enum {
    TVG_STROKE_CAP_SQUARE =
        0, ///< The stroke is extended in both endpoints of a sub-path by a rectangle, with the width equal to the
           ///< stroke width and the length equal to the half of the stroke width. For zero length sub-paths the square
           ///< is rendered with the size of the stroke width.
    TVG_STROKE_CAP_ROUND, ///< The stroke is extended in both endpoints of a sub-path by a half circle, with a radius
                          ///< equal to the half of a stroke width. For zero length sub-paths a full circle is rendered.
    TVG_STROKE_CAP_BUTT   ///< The stroke ends exactly at each of the two endpoints of a sub-path. For zero length
                          ///< sub-paths no stroke is rendered.
} Tvg_Stroke_Cap;
...
typedef enum {
    TVG_STROKE_JOIN_BEVEL =
        0, ///< The outer corner of the joined path segments is bevelled at the join point. The triangular region of the
           ///< corner is enclosed by a straight line between the outer corners of each stroke.
    TVG_STROKE_JOIN_ROUND, ///< The outer corner of the joined path segments is rounded. The circular region is centered
                           ///< at the join point.
    TVG_STROKE_JOIN_MITER ///< The outer corner of the joined path segments is spiked. The spike is created by extension
                          ///< beyond the join point of the outer edges of the stroke until they intersect. In case the
                          ///< extension goes beyond the limit, the join style is converted to the Bevel style.
} Tvg_Stroke_Join;
...
typedef enum {
    ...
    TVG_STROKE_FILL_REFLECT, ///< The gradient pattern is reflected outside the gradient area until the expected region
                             ///< is filled.
    TVG_STROKE_FILL_REPEAT   ///< The gradient pattern is repeated continuously beyond the gradient area until the
                             ///< expected region is filled.
} Tvg_Stroke_Fill;
 ...
 */
typedef enum {
    TVG_FILL_RULE_WINDING = 0, ///< A line from the point to a location outside the shape is drawn. The intersections of
                               ///< the line with the path segment of the shape are counted. Starting from zero, if the
                               ///< path segment of the shape crosses the line clockwise, one is added, otherwise one is
                               ///< subtracted. If the resulting sum is non zero, the point is inside the shape.
    TVG_FILL_RULE_EVEN_ODD     ///< A line from the point to a location outside the shape is drawn and its intersections
                           ///< with the path segments of the shape are counted. If the number of intersections is an
                           ///< odd number, the point is inside the shape.
} Tvg_Fill_Rule;
...
typedef struct {
    ...
    uint8_t
        a; /**< The alpha channel value in the range [0 ~ 255], where 0 is completely transparent and 255 is opaque. */
} Tvg_Color_Stop;
 ...
 * \endcode
 *
 * \param[in] engine_method The engine types to initialize. This is relative to the Canvas types, in which it will be
 * used. For multiple backends bitwise operation is allowed.
 *   - TVG_ENGINE_SW: CPU rasterizer
 *   - TVG_ENGINE_GL: OpenGL rasterizer (not supported yet)
 * \param[in] threads The number of additional threads used to perform rendering. Zero indicates only the main thread is
 * to be used.
 *
 ...
 *
 * \note The Initializer keeps track of the number of times it was called. Threads count is fixed at the first init()
 * call. \see tvg_engine_term() \see Tvg_Engine
 */
 ...
 *
 * It should be called in case of termination of the TVG client with the same engine types as were passed when
 * tvg_engine_init() was called.
 *
 * \code
...
*
* \note A Canvas behavior depends on the raster engine though the final content of the buffer is expected to be
identical.
 ...
 */
typedef enum {
    TVG_COLORSPACE_ABGR8888 = 0, ///< The channels are joined in the order: alpha, blue, green, red. Colors are
                                 ///< alpha-premultiplied. (a << 24 | b << 16 | g << 8 | r)
    TVG_COLORSPACE_ARGB8888,     ///< The channels are joined in the order: alpha, red, green, blue. Colors are
                                 ///< alpha-premultiplied. (a << 24 | r << 16 | g << 8 | b)
    TVG_COLORSPACE_ABGR8888S,    ///< The channels are joined in the order: alpha, blue, green, red. Colors are
                                 ///< un-alpha-premultiplied. @since 0.13
    TVG_COLORSPACE_ARGB8888S     ///< The channels are joined in the order: alpha, red, green, blue. Colors are
                                 ///< un-alpha-premultiplied. @since 0.13
} Tvg_Colorspace;
 ...
 * \retval TVG_RESULT_INVALID_ARGUMENTS An invalid canvas or buffer pointer passed or one of the @p stride, @p w or @p h
 * being zero. \retval TVG_RESULT_NOT_SUPPORTED The software engine is not supported.
 *
 * \warning Do not access @p buffer during tvg_canvas_draw() - tvg_canvas_sync(). It should not be accessed while the
 ...
 *
 * \note When @c policy is set as @c TVG_MEMPOOL_POLICY_INDIVIDUAL, the current instance of canvas uses its own
...
/************************************************************************/
/*!
 * \brief Clears the canvas internal data, releases all paints stored by the canvas and destroys the canvas object
 * itself.
 *
 * \code
 ...
 *
 * \note If the paints from the canvas should not be released, the tvg_canvas_clear() with a @c free argument value set
 * to @c false should be called. Please be aware that in such a case TVG is not responsible for the paints release
 * anymore and it has to be done manually in order to avoid memory leaks.
 *
 ...
 *
 * \note The rendering order of the paints is the same as the order as they were pushed. Consider sorting the paints
 * before pushing them if you intend to use layering. \see tvg_canvas_clear()
 */
 ...
 * Tvg_Paint objects stored in the canvas are released if @p free is set to @c true, otherwise the memory is not
 * deallocated and all paints should be released manually in order to avoid memory leaks.
 *
 ...
 * \retval TVG_RESULT_INSUFFICIENT_CONDITION An internal error.
 *
 * \note Drawing can be asynchronous based on the assigned thread number. To guarantee the drawing is done, call
 * tvg_canvas_sync() afterwards. \see tvg_canvas_sync()
 */
 ...
 * \retval TVG_RESULT_INSUFFICIENT_CONDITION An internal error.
 *
 * \warning It's not allowed to change the viewport during tvg_canvas_update() - tvg_canvas_sync() or tvg_canvas_push()
 * - tvg_canvas_sync().
 *
 ...
 *
 * \warning If this function is used, tvg_canvas_clear() with the @c free argument value set to @c false should be used
 ...
 * \retval TVG_RESULT_INVALID_ARGUMENT An invalid Tvg_Paint pointer.
 *
 * \note Setting the opacity with this API may require multiple renderings using a composition. It is recommended to
 ...
 *
 * \note The bounding box doesn't indicate the actual drawing region. It's the smallest rectangle that encloses the
 * object.
 */
 ...
 * \return Tvg_Result enumeration.
 * \retval TVG_RESULT_SUCCESS Succeed.
 * \retval TVG_RESULT_INVALID_ARGUMENT An invalid @p paint or @p target object or the @p method equal to
 ...
 * lighting, shading, and color mixing, among others. its process involves the combination of colors or images from the
 * source paint object with the destination (the lower layer image) using blending operations. The blending operation is
 * determined by the chosen @p BlendMethod, which specifies how the colors or images are combined.
 *
 * \param[in] paint The Tvg_Paint object of which to get the identifier value.
 ...
 * path. Path can be composed by accumulating primitive commands such as tvg_shape_move_to(), tvg_shape_line_to(),
 * tvg_shape_cubic_to() or complete shape interfaces such as tvg_shape_append_rect(), tvg_shape_append_circle(), etc.
 * Path can consists of sub-paths. One sub-path is determined by a close command.
 *
 * The stroke of a shape is an optional property in case the shape needs to be represented with/without the outline
 ...
 *
 * The Bezier curve starts at the current point and ends at the given end-point (@p x, @p y). Two control points (@p
 * cx1, @p cy1) and (@p cx2, @p cy2) are used to determine the shape of the curve. The value of the current point is set
...
* \retval TVG_RESULT_INVALID_ARGUMENT An invalid Tvg_Paint pointer.
*
& \note For @p rx and @p ry greater than or equal to the half of @p w and the half of @p h, respectively, the shape
become an ellipse.
*/
 ...
 * The current point value is set to the end-point of the arc in case @p pie is @c false, and to the center of the arc
 * otherwise.
 *
 * \param[in] paint A Tvg_Paint pointer to the shape object.
 ...
 * line. \param[in] sweep The central angle of the arc given in degrees, measured counter-clockwise from @p startAngle.
 ...
 * \retval TVG_RESULT_INVALID_ARGUMENT An invalid Tvg_Paint pointer.
 *
 * \note Setting @p sweep value greater than 360 degrees, is equivalent to calling tvg_shape_append_circle(paint, cx,
 * cy, radius, radius).
 */
 ...
 * //TVG approximates a circle by four Bezier curves. In the example above the cmds array stores the commands of the
 * path data. \endcode
 *
 * \param[in] paint A Tvg_Paint pointer to the shape object.
 ...
 * \return Tvg_Result enumeration.
 * \retval TVG_RESULT_SUCCESS Succeed.
 * \retval TVG_RESULT_INVALID_ARGUMENT An invalid pointer passed as an argument and @p cnt > 0, the given length of the
 * array is less than two or any of the @p dashPattern values is zero or less. \retval TVG_RESULT_FAILED_ALLOCATION An
 * internal error with a memory allocation.
 *
 ...
 * TVG_STROKE_JOIN_MITER join style is set. The default value is 4.
 *
 * \return Tvg_Result enumeration.
 * \retval TVG_RESULT_SUCCESS Succeed.
 * \retval TVG_RESULT_INVALID_ARGUMENT An invalid Tvg_Paint pointer.
 ...
 * \param[in] g The green color channel value in the range [0 ~ 255]. The default value is 0.
 * \param[in] b The blue color channel value in the range [0 ~ 255]. The default value is 0.
 * \param[in] a The alpha channel value in the range [0 ~ 255], where 0 is completely transparent and 255 is opaque. The
 * default value is 0.
 *
 * \return Tvg_Result enumeration.
 * \retval TVG_RESULT_SUCCESS Succeed.
 ...
 *
 * \brief A module enabling to create and to load an image in one of the supported formats: svg, png, jpg, lottie and
 * raw.
 *
 *
 * \{
 ...
 * \return Tvg_Result enumeration.
 * \retval TVG_RESULT_SUCCESS Succeed.
 * \retval TVG_RESULT_INVALID_ARGUMENT An invalid Tvg_Paint pointer or no data are provided or the @p width or @p height
 * value is zero or less. \retval TVG_RESULT_FAILED_ALLOCATION A problem with memory allocation occurs. \retval
 ...
 * \param[in] mimetype Mimetype or extension of data such as "jpg", "jpeg", "svg", "svg+xml", "lottie", "png", etc. In
 * case an empty string or an unknown type is provided, the loaders will be tried one by one. \param[in] copy If @c true
 ...
 *
 * Tvg_Paint objects stored in the scene are released if @p free is set to @c true, otherwise the memory is not
 * deallocated and all paints should be released manually in order to avoid memory leaks.
 *
 ...
 *
 * \note Saving can be asynchronous if the assigned thread number is greater than zero. To guarantee the saving is done,
 * call tvg_saver_sync() afterwards. \see tvg_saver_sync()
 */
 ...
 * Thus, if you wish to have a benefit of it, you must call tvg_saver_sync() after the tvg_saver_save() in the proper
 * delayed time. Otherwise, you can call tvg_saver_sync() immediately.
 *
 * \param[in] saver The Tvg_Saver object connected with the saving task.
 *
 * \return Tvg_Result enumeration.
 ...
 * \param[in] no The index of the animation frame to be displayed. The index should be less than the
 * tvg_animation_total_frame().
 *
 * \return Tvg_Result enumeration.
 * \retval TVG_RESULT_SUCCESS Succeed.
