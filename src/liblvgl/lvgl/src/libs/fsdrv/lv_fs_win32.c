...
static void *fs_open(lv_fs_drv_t *drv, const char *path, lv_fs_mode_t mode)
{
    ...
    return (void *)CreateFileA(buf, desired_access, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
}
...
static lv_fs_res_t fs_seek(lv_fs_drv_t *drv, void *file_p, uint32_t pos, lv_fs_whence_t whence)
{
    ...
    return SetFilePointerEx((HANDLE)file_p, distance_to_move, NULL, move_method) ? LV_FS_RES_OK
