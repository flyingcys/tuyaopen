...
static int32_t grid_align(int32_t cont_size, bool auto_size, lv_grid_align_t align, int32_t gap, uint32_t track_num,
...
{
    return lv_obj_get_style_margin_left(obj, LV_PART_MAIN) + lv_obj_get_style_margin_right(obj, LV_PART_MAIN);
}
static inline int32_t get_margin_ver(lv_obj_t *obj)
{
    return lv_obj_get_style_margin_top(obj, LV_PART_MAIN) + lv_obj_get_style_margin_bottom(obj, LV_PART_MAIN);
}
...
static void calc(lv_obj_t *cont, lv_grid_calc_t *calc_out)
{
    ...
    calc_out->grid_w =
        grid_align(cont_w, auto_w, get_grid_col_align(cont), col_gap, calc_out->col_num, calc_out->w, calc_out->x, rev);
...
static void item_repos(lv_obj_t *item, lv_grid_calc_t *c, item_repos_hint_t *hint)
{
    ...
    case LV_GRID_ALIGN_CENTER:
        x = c->x[col_pos] + (col_w - item_w) / 2 +
            (lv_obj_get_style_margin_left(item, LV_PART_MAIN) - lv_obj_get_style_margin_right(item, LV_PART_MAIN)) / 2;
        item->w_layout = 0;
        break;
    ...
    case LV_GRID_ALIGN_CENTER:
        y = c->y[row_pos] + (row_h - item_h) / 2 +
            (lv_obj_get_style_margin_top(item, LV_PART_MAIN) - lv_obj_get_style_margin_bottom(item, LV_PART_MAIN)) / 2;
        item->h_layout = 0;
        break;
    case LV_GRID_ALIGN_END:
