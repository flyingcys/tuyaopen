...
{
    const char *txt =
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla nec rhoncus arcu, in consectetur orci. Sed "
        "vitae dolor sed nisi ultrices vehicula quis ac dolor. Vivamus hendrerit hendrerit lectus, sed tempus velit "
        "suscipit in. Fusce eu tristique arcu. Sed et molestie leo, in lacinia nunc. Quisque semper lorem sed ante "
        "feugiat, at molestie risus blandit. Maecenas lobortis urna in diam feugiat porta. Ut facilisis mauris eget "
        "nibh posuere aliquet. Proin facilisis egestas magna, id vulputate massa bibendum a. Etiam gravida metus non "
        "egestas suscipit. Sed sollicitudin mollis nisi, eu fringilla leo vestibulum posuere. Donec et ex nulla. "
        "Phasellus et ornare justo, vel hendrerit justo. Curabitur pulvinar nunc sed tincidunt dignissim. Praesent "
        "eleifend lectus velit, id malesuada ante placerat id. Fusce massa erat, egestas vel venenatis eu, tempus nec "
        "est.\n\n"
        "Phasellus iaculis malesuada molestie. Cras ullamcorper justo a dolor dignissim tincidunt. Mauris euismod "
        "risus quis lobortis mollis. Ut vitae placerat massa, aliquet various lectus. Nulla ac ornare purus, quis "
        "auctor velit. Donec posuere dolor rhoncus efficitur dictum. Integer venenatis aliquet nunc eu convallis. Nunc "
        "quis various velit. Suspendisse enim metus, molestie eget mauris sit amet, euismod volutpat turpis. Duis "
        "rhoncus commodo gravida. Pellentesque velit mi, dictum id consequat placerat, condimentum ac elit. Duis "
        "aliquet leo eu dolor cursus rhoncus. Quisque aliquam sapien ut purus hendrerit laoreet. Ut venenatis "
        "venenatis risus, a vestibulum enim lobortis a. Maecenas auctor tortor lorem, quis laoreet nulla aliquet a. "
        "Sed ipsum lorem, facilisis in congue a, dictum ut ligula.\n\n"
        "Aliquam id tellus in enim hendrerit mattis. Sed ipsum arcu, feugiat sed eros quis, vulputate facilisis "
        "turpis. Quisque venenatis risus massa. Proin lacinia, nunc non ultrices commodo, ligula dolor lobortis "
        "lectus, iaculis pulvinar metus orci eu elit. Donec tincidunt lacinia semper. Class aptent taciti sociosqu ad "
        "litora torquent per conubia nostra, per inceptos himenaeos. Donec vitae odio risus. Donec sodales sed nulla "
        "sit amet iaculis. Duis lacinia mauris dictum, fermentum nibh eget, convallis tellus. Sed congue luctus purus "
        "non scelerisque. Etiam fermentum lacus mauris, at bibendum nunc aliquam at. Vivamus accumsan vestibulum "
        "pharetra. Proin rhoncus nisi purus, vel blandit metus auctor eget. Fusce dictum sed lectus sed aliquam. "
        "Praesent lobortis quam sed pretium tincidunt.\n\n"
        "Integer vehicula vestibulum eros. Donec facilisis magna a est cursus, sed posuere velit faucibus. In et "
        "ultrices lorem. Sed et lacus finibus, vulputate odio et, finibus tellus. Aenean finibus nibh vehicula "
        "elementum maximus. Maecenas in luctus tortor, vitae ullamcorper lacus. Ut nulla elit, aliquam at vestibulum "
        "ut, pellentesque non justo.\n\n"
        "Fusce dignissim turpis massa, eget semper purus semper at. Ut et augue vitae metus laoreet auctor. Morbi "
        "tincidunt, neque vel tincidunt interdum, sapien nibh finibus lorem, eu eleifend diam ipsum et eros. Duis "
        "iaculis vulputate lacinia. Phasellus id mauris sed magna gravida suscipit. Sed aliquet tincidunt ante ac "
        "posuere. In vestibulum quam ultricies, ultricies arcu eu, aliquam sapien. Phasellus sollicitudin velit "
        "facilisis, dignissim nisi sed, pellentesque magna.";
...
static void sysmon_perf_observer_cb(lv_observer_t *observer, lv_subject_t *subject)
{
    ...
    lv_label_set_text_fmt(label,
                          ...
                          scene_name, info->calculated.fps, info->calculated.cpu,
...
static void summary_create(void)
{
    ...
    for (i = 0; scenes[i].create_cb; i++) {
        ...
        if (scenes[i].measurement_cnt <= 1) {
            ...
            LV_LOG("%s, %" LV_PRIu32 "%%, %" LV_PRIu32 ", %" LV_PRIu32 ", %" LV_PRIu32 ", %" LV_PRIu32 "\r\n",
                   scenes[i].name, scenes[i].cpu_avg_usage / cnt, scenes[i].fps_avg / cnt, render_time + flush_time,
    ...
    if (valid_scene_cnt < 1) {
        ...
        LV_LOG("All scenes avg.,%" LV_PRIu32 "%%, %" LV_PRIu32 ", %" LV_PRIu32 ", %" LV_PRIu32 ", %" LV_PRIu32 "\r\n",
               total_avg_cpu / valid_scene_cnt, total_avg_fps / valid_scene_cnt, render_time + flush_time, render_time,
               flush_time);
    }
}
