...
#define SPINBOX_EVENT_ATTACH(item)                                                                                     \
    lv_obj_add_event_cb(ui->ctrl_pad.tab.layout.spinbox_##item, ctrl_pad_spinbox_event_handler,                        \
...
void ctrl_pad_attach(view_t *ui)
{
    ...
    lv_obj_add_event_cb(ui->ctrl_pad.tab.flex.checkbox_scrl, ctrl_pad_checkbox_event_handler, LV_EVENT_VALUE_CHANGED,
                        ui);
}
...
static void ctrl_pad_spinbox_event_handler(lv_event_t *e)
{
    ...
    lv_style_value_t v = {.num = (int32_t)lv_spinbox_get_value(spinbox)};
...
static void ctrl_pad_checkbox_event_handler(lv_event_t *e)
{
    ...
    if (ui->obj_cur) {
        ...
        checked ? lv_obj_add_flag(ui->obj_cur, LV_OBJ_FLAG_SCROLLABLE)
                : lv_obj_remove_flag(ui->obj_cur, LV_OBJ_FLAG_SCROLLABLE);
    }
}

static void ctrl_pad_btn_event_attach(view_t *ui)
{
    lv_obj_add_event_cb(ui->ctrl_pad.btn.add, ctrl_pad_btn_add_event_handler, LV_EVENT_CLICKED, ui);

    lv_obj_add_event_cb(ui->ctrl_pad.btn.remove, ctrl_pad_btn_remove_event_handler, LV_EVENT_CLICKED, ui);
}
