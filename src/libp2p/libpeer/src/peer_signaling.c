...
static int peer_signaling_resolve_url(const char* url, char* host, int* port, char* path) {
  ...
  PR_NOTICE("Host: %s, Port: %d, Path: %s", host, *port, path);
  return proto;
}
...
static void peer_signaling_mqtt_publish(MQTTContext_t* mqtt_ctx, const char* message) {
  ...
  if (status != MQTTSuccess) {
    PR_ERR("MQTT_Publish failed: Status=%s.", MQTT_Status_strerror(status));
  } else {
    PR_DEBUG("MQTT_Publish succeeded.");
  }
}
...
HTTPResponse_t peer_signaling_http_request(const TransportInterface_t* transport_interface,
  ...
  if (status == HTTPSuccess) {
    ...
    PR_ERR("Failed to initialize HTTP request headers: Error=%s.", HTTPClient_strerror(status));
  }
...
static int peer_signaling_http_post(const char* hostname, const char* path, int port, const char* auth, const char* body) {
  ...
  if (ret < 0) {
    PR_ERR("Failed to connect to %s:%d", hostname, port);
    return ret;
  }
...
static void peer_signaling_mqtt_event_cb(MQTTContext_t* mqtt_ctx,
  ...
  switch (packet_info->type) {
    case MQTT_PACKET_TYPE_PUBLISH:
      PR_DEBUG("MQTT received message: %.*s",
    ...
    case MQTT_PACKET_TYPE_SUBACK: {
      ...
      for (i = 0; i < ncodes; i++) {
        if (codes[0] == MQTTSubAckFailure) {
          PR_ERR("MQTT Subscription failed. Please check authorization");
          break;
        }
      }

      if (i == ncodes) {
        PR_NOTICE("MQTT Subscribe succeeded.");
      }
    } break;
    case MQTT_PACKET_TYPE_UNSUBACK:
      PR_NOTICE("MQTT Unsubscribe succeeded.");
      break;
    default:
      break;
  }
...
static int peer_signaling_mqtt_connect(const char* hostname, int port) {
  ...
  if (ssl_transport_connect(&g_ps.net_ctx, hostname, port, NULL) < 0) {
    PR_ERR("ssl transport connect failed");
    return -1;
  }
  ...
  if (status != MQTTSuccess) {
    PR_ERR("MQTT_Connect failed: Status=%s.", MQTT_Status_strerror(status));
    return -1;
  }

  PR_NOTICE("MQTT_Connect succeeded.");
  return 0;
}
...
static int peer_signaling_mqtt_subscribe(int subscribed) {
  ...
  if (status != MQTTSuccess) {
    PR_ERR("MQTT_ProcessLoop failed: Status=%s.", MQTT_Status_strerror(status));
    return -1;
  }
...
void peer_signaling_disconnect() {
  ...
  if (!g_ps.proto && !peer_signaling_mqtt_subscribe(0)) {
    ...
    if (status != MQTTSuccess) {
      PR_ERR("Failed to disconnect with broker: %s", MQTT_Status_strerror(status));
    }
