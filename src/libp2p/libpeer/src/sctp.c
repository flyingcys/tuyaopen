...
int sctp_outgoing_data(Sctp* sctp, char* buf, size_t len, SctpDataPpid ppid, uint16_t sid) {
  ...
  if (res < 0) {
    PR_ERR("sctp sendv error %d: %s", errno, strerror(errno));
  }
  return res;
#else
...
void sctp_add_stream_mapping(Sctp* sctp, const char* label, uint16_t sid) {
  if (sctp->stream_count < SCTP_MAX_STREAMS) {
    ...
    PR_ERR("Stream table full. Cannot add more streams.");
}
...
void sctp_incoming_data(Sctp* sctp, char* buf, size_t len) {
  ...
  // Header
#if 0
  PR_DEBUG("source_port %d", ntohs(in_packet->header.source_port));
  PR_DEBUG("destination_port %d", ntohs(in_packet->header.destination_port));
  PR_DEBUG("verification_tag %ld", ntohl(in_packet->header.verification_tag));
  PR_DEBUG("checksum %d", ntohs(in_packet->header.checksum));
#endif
  ...
  while ((4 * (pos + 3) / 4) < len) {
    ...
    switch (chunk_common->type) {
      case SCTP_DATA: {
        ...
        PR_DEBUG("SCTP_DATA. ppid = %ld, data = %.2x", ntohl(data_chunk->ppid), data_chunk->data[0]);
      ...
      case SCTP_SACK:
        ...
        PR_DEBUG("cumulative_tsn_ack %d", ntohl(sack->cumulative_tsn_ack));
        PR_DEBUG("a_rwnd %d", ntohl(sack->a_rwnd));
        PR_DEBUG("number_of_gap_ack_blocks %d", sack->number_of_gap_ack_blocks);
        PR_DEBUG("number_of_dup_tsns %d", sack->number_of_dup_tsns);
#endif
// XXX: unordered sequence
        ...
        if (sack->number_of_gap_ack_blocks > 0) {
          ...
          PR_NOTICE("cumulative_tsn_ack: %ld, number_of_gap_ack_blocks: %d",
          ...
          for (int i = 0; i < blocks; i++) {
            ...
            PR_NOTICE("start: %d, end: %d", ntohs(*start), ntohs(*end));
            sctp->tsn = ntohl(sack->cumulative_tsn_ack) + 1;// + (*start) - 1;
          }
        } else if (sack->number_of_dup_tsns > 0) {

          PR_NOTICE("cumulative_tsn_ack: %ld, number_of_dup_tsns: %d",
      ...
      case SCTP_ABORT:
        ...
        break;
      default:
        PR_NOTICE("Unknown chunk type %d", chunk_common->type);
        length = 0;
        break;
    }
...
static int sctp_handle_incoming_data(Sctp* sctp, char* data, size_t len, uint32_t ppid, uint16_t sid, int flags) {
  ...
  switch (ppid) {
    ...
    case DATA_CHANNEL_PPID_BINARY_PARTIAL:
      ...
      PR_DEBUG("Got message (size = %ld)", len);
      if (sctp->onmessage) {
...
static int sctp_incoming_data_cb(struct socket* sock, union sctp_sockstore addr, void* data, size_t len, struct sctp_rcvinfo recv_info, int flags, void* userdata) {
  Sctp* sctp = (Sctp*)userdata;
  PR_DEBUG("Data of length %u received on stream %u with SSN %u, TSN %u, PPID %u",
       (uint32_t)len,
       recv_info.rcv_sid,
       recv_info.rcv_ssn,
       recv_info.rcv_tsn,
  ...
  tal_tal_free(data);  // we need to free the memory that usrsctp allocates
  return 0;
}
#endif
...
int sctp_create_association(Sctp* sctp, DtlsSrtp* dtls_srtp) {
  ...
  if (!sock) {
    PR_ERR("usrsctp_socket failed");
    return -1;
  }

  do {
    if (usrsctp_set_non_blocking(sock, 1) < 0) {
      PR_ERR("usrsctp_set_non_blocking failed");
      break;
    }
