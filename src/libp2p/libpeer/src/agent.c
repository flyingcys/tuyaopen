...
int agent_create(Agent* agent) {
  ...
  PR_NOTICE("create IPv4 UDP socket: %d", agent->udp_sockets[0].fd);

#if CONFIG_IPV6
  if ((ret = udp_socket_open(&agent->udp_sockets[1], AF_INET6, 0)) < 0) {
    PR_ERR("Failed to create IPv6 UDP socket.");
    return ret;
  }
  PR_NOTICE("create IPv6 UDP socket: %d", agent->udp_sockets[1].fd);
#endif
...
static int agent_create_turn_addr(Agent* agent, Address* serv_addr, const char* username, const char* credential) {
  ...
  ret = agent_socket_send(agent, serv_addr, send_msg.buf, send_msg.size);
  if (ret == -1) {
    PR_ERR("Failed to send TURN Binding Request.");
    return -1;
  }
  ...
  if (recv_msg.stunclass == STUN_CLASS_ERROR && recv_msg.stunmethod == STUN_METHOD_ALLOCATE) {
    ...
    PR_ERR("Invalid TURN Binding Response.");
    return -1;
  }
  ...
  if (ret <= 0) {
    PR_DEBUG("Failed to receive TURN Binding Response.");
    return ret;
  }
...
void agent_gather_candidate(Agent* agent, const char* urls, const char* username, const char* credential) {
  ...
  for (i = 0; i < sizeof(addr_type) / sizeof(addr_type[0]); i++) {
    if (ports_resolve_addr(hostname, &resolved_addr) == 0) {
      ...
      PR_NOTICE("Resolved stun/turn server %s:%d", addr_string, port);

      if (strncmp(urls, "stun:", 5) == 0) {
        PR_DEBUG("Create stun addr");
        agent_create_stun_addr(agent, &resolved_addr);
      } else if (strncmp(urls, "turn:", 5) == 0) {
        PR_DEBUG("Create turn addr");
...
void agent_set_remote_description(Agent* agent, char* description) {
  ...
  */
  ...
  PR_DEBUG("Set remote description:\n%s", description);
  ...
  PR_DEBUG("remote ufrag: %s", agent->remote_ufrag);
  PR_DEBUG("remote upwd: %s", agent->remote_upwd);
  ...
  PR_DEBUG("candidate pairs num: %d", agent->candidate_pairs_num);
}
...
int agent_connectivity_check(Agent* agent) {
  ...
  if (agent->nominated_pair->state != ICE_CANDIDATE_STATE_INPROGRESS) {
    PR_NOTICE("nominated pair is not in progress");
    return -1;
  }
  ...
  if (agent->nominated_pair->conncheck % AGENT_CONNCHECK_PERIOD == 0) {
    ...
    PR_DEBUG("send binding request to remote ip: %s, port: %d", addr_string, agent->nominated_pair->remote->addr.port);
