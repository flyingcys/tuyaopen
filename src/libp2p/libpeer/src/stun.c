...
int stun_set_mapped_address(char* value, uint8_t* mask, Address* addr) {
  ...
  *port = htons(addr->port) ^ *(uint16_t*)mask;
  ...
  PR_DEBUG("XOR Mapped Address Family: %d", *family);
  PR_DEBUG("XOR Mapped Address Port: %d (Port XOR: %04x)", addr->port, *port);
  // PR_DEBUG("XOR Mapped Address IP: %s (IP XOR: %08" PRIu32 ")", addr_string, *addr32);
  return ret;
}
...
void stun_parse_msg_buf(StunMessage* msg) {
  ...
  while (pos < length) {
    ...
    // PR_DEBUG("Attribute Type: 0x%04x", ntohs(attr->type));
    // PR_DEBUG("Attribute Length: %d", ntohs(attr->length));
    ...
    switch (ntohs(attr->type)) {
      ...
      case STUN_ATTR_TYPE_USERNAME:
        ...
        // PR_DEBUG("length = %d, Username %s", ntohs(attr->length), msg->username);
        break;
      ...
      case STUN_ATTR_TYPE_NETWORK_COST:
        ...
        break;
      default:
        PR_ERR("Unknown Attribute Type: 0x%04x", ntohs(attr->type));
        break;
    }
...
int stun_probe(uint8_t* buf, size_t size) {
  ...
  if (size < sizeof(StunHeader)) {
    PR_ERR("STUN message is too short.");
    return -1;
  }
...
StunMsgType stun_is_stun_msg(uint8_t *buf, size_t size) {
  ...
  if (header->magic_cookie != htonl(MAGIC_COOKIE)) {
    //PR_ERR("STUN magic cookie does not match.");
    return STUN_MSG_TYPE_INVLID;
  }
...
int stun_msg_is_valid(uint8_t* buf, size_t size, char* password) {
  ...
  // PR_DEBUG("Fingerprint: 0x%08x", fingerprint);

  if (fingerprint != msg.fingerprint) {
    // PR_ERR("Fingerprint does not match.");
    return -1;
  } else {
    // PR_DEBUG("Fingerprint matches.");
  }
  ...
  // PR_DEBUG("message_integrity: 0x%s", message_integrity_hex);

  if (memcmp(message_integrity, msg.message_integrity, 20) != 0) {
    // PR_ERR("Message Integrity does not match.");
    return -1;
  } else {
    // PR_DEBUG("Message Integrity matches.");
  }
