...
static void *fuzz_alloc(const size_t size, const bool do_zero)
{
    ...
    if (size == 0) {
         ...
         * fuzz_tal_free is aware of this pointer range and will not attempt
         * to tal_free()/munmap() it.
         */
...
static srtp_policy_t *extract_policy(const uint8_t **data, size_t *size)
{
    ...
    if ((params.do_extract_master_keys % 2) == 0) {
        ...
        if (policy->keys == NULL) {
            fuzz_tal_free(policy->key);
            fuzz_tal_free(policy->enc_xtn_hdr);
            fuzz_tal_free(policy);
            return NULL;
        }
    }
...
static void tal_free_policies(srtp_policy_t *curpolicy)
{
    size_t i;
    while (curpolicy) {
        srtp_policy_t *next = curpolicy->next;

        fuzz_tal_free(curpolicy->key);

        for (i = 0; i < curpolicy->num_master_keys; i++) {
            fuzz_tal_free(curpolicy->keys[i]->key);
            fuzz_tal_free(curpolicy->keys[i]->mki_id);
            fuzz_tal_free(curpolicy->keys[i]);
        }

        fuzz_tal_free(curpolicy->keys);
        fuzz_tal_free(curpolicy->enc_xtn_hdr);
        fuzz_tal_free(curpolicy);
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
...
#ifdef FUZZ_32BIT
    /* tal_free the mmap allocation made during the previous iteration, if
     * applicable */
    fuzz_tal_free(g_mmap_allocation);
#endif
    ...
    // tal_free_policies(policy_chain);
    // policy_chain = NULL;
...

    {
        ...
        while ((ret = run_srtp_func(srtp_ctx, &data, &size)) != NULL) {
            ...
            if (policy_chain_2 != NULL) {
                ...
                tal_free_policies(policy_chain_2);
                policy_chain_2 = NULL;
            }
        }
    }
    ...
    tal_free_policies(policy_chain);
    tal_free_policies(policy_chain_2);
    fuzz_tal_free(remove_stream_ssrc);
    fuzz_tal_free(set_roc);
    if (srtp_ctx != NULL) {
