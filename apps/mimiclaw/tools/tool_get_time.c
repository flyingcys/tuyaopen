...
static bool find_header_value(const char *headers, size_t headers_len,
...
static void format_local_time(TIME_T epoch, char *out, size_t out_size)
{
    ...
    snprintf(out, out_size, "%04d-%02d-%02d %02d:%02d:%02d %s (%s)",
             tm_local.tm_year + 1900, tm_local.tm_mon + 1, tm_local.tm_mday,
             tm_local.tm_hour, tm_local.tm_min, tm_local.tm_sec, tz_label, wday);
}
...
static OPERATE_RET fetch_date_direct(char *date_buf, size_t date_buf_size)
{
    ...
    for (size_t i = 0; i < sizeof(methods) / sizeof(methods[0]); i++) {
        ...
        if (response.headers && response.headers_length > 0) {
            ok = find_header_value((const char *)response.headers, response.headers_length,
                                   "Date", date_buf, date_buf_size);
        }
...
OPERATE_RET tool_get_time_execute(const char *input_json, char *output, size_t output_size)
{
    ...
    MIMI_LOGI(TAG, "time synced via %s, date=%s, local=%s",
              http_proxy_is_enabled() ? "proxy" : "direct", date_val, output);
    return OPRT_OK;
}
