            ...
            cJSON_ArrayForEach(block, content) {
                ...
                if (strcmp(btype->valuestring, "tool_result") == 0) {
                    ...
                    cJSON_AddStringToObject(tm, "content",
                                            cJSON_IsString(tcontent) ? tcontent->valuestring : "");
...
static OPERATE_RET llm_http_call(const char *post_data, char *resp_buf, size_t resp_buf_size,
...
OPERATE_RET llm_chat_tools(const char *system_prompt,
                           cJSON *messages,
