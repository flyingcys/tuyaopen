...
static OPERATE_RET refresh_tenant_token(void)
{
    ...
    uint16_t status = 0;
    OPERATE_RET rt = fs_http_call(FS_HOST,
                                  "/open-apis/auth/v3/tenant_access_token/internal",
                                  "POST",
                                  json,
                                  NULL,
                                  resp,
                                  FS_HTTP_RESP_BUF_SIZE,
                                  &status);
    cJSON_free(json);
    ...
    if (cJSON_IsNumber(code) && (int)code->valuedouble == 0 && cJSON_IsObject(data)) {
        ...
        if (url && url[0] != '\0') {
            ...
            if (cJSON_IsObject(cc)) {
                ...
                int ri = json_int2(cc, "ReconnectInterval", "reconnect_interval", (int)(conf->reconnect_interval_ms / 1000));
...
static OPERATE_RET fs_parse_ws_url(const char *url,
...
static OPERATE_RET fs_ws_decode_one_frame(fs_ws_conn_t *conn,
                                          uint8_t *opcode,
                                          uint8_t **payload,
                                          size_t *payload_len,
                                          size_t *consumed)
...
static OPERATE_RET fs_ws_poll_frame(fs_ws_conn_t *conn,
                                    int wait_ms,
                                    uint8_t *opcode,
                                    uint8_t **payload,
                                    size_t *payload_len)
...
static bool fs_pb_encode_frame(const fs_pb_frame_t *frame, uint8_t **out, size_t *out_len)
{
    ...
    for (size_t i = 0; i < frame->header_count; i++) {
        ...
        if (!pb_write_key(&b, 5, 2) || !pb_write_varint(&b, (uint64_t)hb.len) ||
            !fs_buf_append(&b, hb.data, hb.len)) {
            free(hb.data);
            goto fail;
        }
...
static OPERATE_RET fs_frag_merge(const char *message_id,
                                 uint32_t sum,
                                 uint32_t seq,
...
static void extract_message_text(const char *msg_type, const char *content_json, char *out, size_t out_size)
{
    ...
    } else if (strcmp(msg_type, "image") == 0 || strcmp(msg_type, "audio") == 0 ||
               strcmp(msg_type, "file") == 0 || strcmp(msg_type, "media") == 0 ||
               strcmp(msg_type, "sticker") == 0) {
        snprintf(out, out_size, "[%s]", msg_type);
    } else {
        snprintf(out, out_size, "[%s]", msg_type);
    }
...
static OPERATE_RET handle_data_pb_frame(fs_ws_conn_t *conn, const fs_pb_frame_t *frame)
{
    ...
    OPERATE_RET rt = fs_frag_merge(msg_id, sum, seq,
                                   frame->payload ? frame->payload : (const uint8_t *)"",
                                   frame->payload_len,
                                   &payload,
                                   &payload_len);
    if (rt == OPRT_RESOURCE_NOT_READY) {
