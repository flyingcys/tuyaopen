...
#define DC_HOST MIMI_DC_API_HOST
...
static OPERATE_RET dc_ws_handshake(dc_gateway_conn_t *conn)
{
    ...
    int req_len = snprintf(req, sizeof(req),
                           "GET " MIMI_DC_GATEWAY_PATH " HTTP/1.1\r\n"
                           "Host: " MIMI_DC_GATEWAY_HOST "\r\n"
                           "Upgrade: websocket\r\n"
                           "Connection: Upgrade\r\n"
                           "Sec-WebSocket-Key: %s\r\n"
...
static OPERATE_RET dc_ws_decode_one_frame(dc_gateway_conn_t *conn,
                                          uint8_t *opcode,
                                          uint8_t **payload,
                                          size_t *payload_len,
                                          size_t *consumed)
...
static OPERATE_RET dc_ws_poll_frame(dc_gateway_conn_t *conn,
                                    int wait_ms,
                                    uint8_t *opcode,
                                    uint8_t **payload,
                                    size_t *payload_len)
...
static OPERATE_RET dc_gateway_send_identify(dc_gateway_conn_t *conn)
{
    ...
    int n = snprintf(payload, sizeof(payload),
                     "{\"op\":2,\"d\":{\"token\":\"%s\",\"intents\":%u,\"properties\":{\"os\":\"tuyaopen\",\"browser\":\"mimiclaw\",\"device\":\"mimiclaw\"}}}",
                     s_bot_token, (unsigned)MIMI_DC_GATEWAY_INTENTS);
    if (n <= 0 || n >= (int)sizeof(payload)) {
        return OPRT_BUFFER_NOT_ENOUGH;
    }

    MIMI_LOGI(TAG, "discord gateway identify sent intents=%u", (unsigned)MIMI_DC_GATEWAY_INTENTS);
...
static OPERATE_RET handle_gateway_payload(dc_gateway_conn_t *conn,
                                          const char *json_str,
                                          int64_t *seq,
    ...
    if (op == 10) {
        ...
        if (cJSON_IsNumber(hb) && hb->valuedouble > 0) {
            ...
            *next_heartbeat_ms = tal_system_get_millisecond() + *heartbeat_ms;
            MIMI_LOGI(TAG, "discord gateway HELLO heartbeat=%u ms", (unsigned)*heartbeat_ms);
        }

        rt = dc_gateway_send_identify(conn);
    } else if (op == 11) {
        MIMI_LOGD(TAG, "discord gateway HEARTBEAT_ACK");
    } else if (op == 1) {
        ...
        MIMI_LOGW(TAG, "discord gateway requested reconnect op=%d", op);
        rt = OPRT_COM_ERROR;
    } else if (op == 0) {
        ...
        if (event_type && strcmp(event_type, "READY") == 0) {
            ...
            MIMI_LOGI(TAG, "discord gateway READY user=%s(%s)", uname ? uname : "", uid ? uid : "");
...
static void discord_gateway_task(void *arg)
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (rt != OPRT_OK) {
                free(payload);
                MIMI_LOGW(TAG, "discord gateway poll failed rt=%d", rt);
                break;
            }
...
static OPERATE_RET dc_http_call_via_proxy(const char *path, const char *method, const char *post_data,
...
static OPERATE_RET dc_http_call(const char *path, const char *method, const char *post_data,
                                char *resp_buf, size_t resp_buf_size, uint16_t *status_code)
{
...
static void parse_message_id(const char *json_str, char *msg_id, size_t msg_id_size)
{
    ...
    if (id && id[0] != '\0') {
        safe_copy(msg_id, msg_id_size, id);
    }
    cJSON_Delete(root);
}
...
{
    if (MIMI_SECRET_DC_TOKEN[0] != '\0') {
        safe_copy(s_bot_token, sizeof(s_bot_token), MIMI_SECRET_DC_TOKEN);
    }
    if (MIMI_SECRET_DC_CHANNEL_ID[0] != '\0') {
        safe_copy(s_channel_id, sizeof(s_channel_id), MIMI_SECRET_DC_CHANNEL_ID);
    }

    char tmp[160] = {0};
    if (mimi_kv_get_string(MIMI_NVS_DC, MIMI_NVS_KEY_DC_TOKEN, tmp, sizeof(tmp)) == OPRT_OK && tmp[0] != '\0') {
        safe_copy(s_bot_token, sizeof(s_bot_token), tmp);
    }

    memset(tmp, 0, sizeof(tmp));
    if (mimi_kv_get_string(MIMI_NVS_DC, MIMI_NVS_KEY_DC_CHANNEL_ID, tmp, sizeof(tmp)) == OPRT_OK && tmp[0] != '\0') {
...
OPERATE_RET discord_bot_start(void)
{
    ...
    THREAD_CFG_T cfg = {0};
    cfg.stackDepth = MIMI_DC_POLL_STACK;
    cfg.priority = THREAD_PRIO_1;
    cfg.thrdname = "mimi_dc_gw";

    OPERATE_RET rt = tal_thread_create_and_start(&s_gateway_thread, NULL, NULL,
                                                 discord_gateway_task, NULL, &cfg);
    if (rt != OPRT_OK) {
        MIMI_LOGE(TAG, "create discord gateway thread failed: %d", rt);
        return rt;
    }
...
OPERATE_RET discord_send_message(const char *channel_id, const char *text)
{
    ...
    while (offset < text_len || (text_len == 0 && offset == 0)) {
        ...
        if (chunk > MIMI_DC_MAX_MSG_LEN) {
            chunk = MIMI_DC_MAX_MSG_LEN;
        }
        if (text_len == 0) {
            chunk = 0;
        }
        ...
        char path[320] = {0};
        snprintf(path, sizeof(path), MIMI_DC_API_BASE "/channels/%s/messages", target_channel);

        char *resp = tal_malloc(DC_HTTP_RESP_BUF_SIZE);
        if (!resp) {
            cJSON_free(json);
            free(segment);
            return OPRT_MALLOC_FAILED;
        ...
        if (rt == OPRT_OK && (status == 200 || status == 201)) {
            ...
            MIMI_LOGI(TAG, "discord send success channel=%s bytes=%u message_id=%s",
                      target_channel, (unsigned)chunk, msg_id[0] ? msg_id : "unknown");
        } else if (status == 429) {
            uint32_t retry_ms = parse_retry_after_ms(resp);
            if (retry_ms == 0) {
                retry_ms = 2000;
            }
            MIMI_LOGW(TAG, "discord send rate limited, retry in %u ms", (unsigned)retry_ms);
...
OPERATE_RET discord_set_token(const char *token)
{
    ...
    return mimi_kv_set_string(MIMI_NVS_DC, MIMI_NVS_KEY_DC_TOKEN, token);
}
...
OPERATE_RET discord_set_channel_id(const char *channel_id)
{
    ...
    safe_copy(s_channel_id, sizeof(s_channel_id), channel_id);
    return mimi_kv_set_string(MIMI_NVS_DC, MIMI_NVS_KEY_DC_CHANNEL_ID, channel_id);
}
