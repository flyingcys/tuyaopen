...
static void        xz_on_listen_start_locked(const char *mode);
static void        xz_on_listen_stop_locked(void);
static void        xz_on_tts_start_locked(void);
static void        xz_on_tts_stop_locked(void);
static void        xz_on_abort_locked(void);
...
static void xz_on_listen_start_locked(const char *mode)
{
    ...
    xz_audio_start_capture_locked();
}

static void xz_on_listen_stop_locked(void)
{
    xz_set_chat_state_locked(XZ_CHAT_IDLE);
    xz_audio_stop_capture_locked();
}
...
static void xz_on_tts_stop_locked(void)
{
    xz_set_chat_state_locked(xz_state_after_tts_stop(s_app.chat_state));
    xz_audio_stop_capture_locked();
    xz_audio_abort_playback_locked();
}
    ...
    if (strcmp(type->valuestring, "tts") == 0) {
        ...
        if (cJSON_IsString(state) && state->valuestring) {
            if (strcmp(state->valuestring, "start") == 0) {
                xz_on_tts_start_locked();
            } else if (strcmp(state->valuestring, "stop") == 0) {
                xz_on_tts_stop_locked();
...
OPERATE_RET xiaozhi_app_send_listen(const char *state, const char *mode, const char *text)
{
    ...
    (void)tal_mutex_lock(s_app.lock);
    if (s_app.active == XZ_ACTIVE_WS) {
        rt = xz_ws_send_listen(&s_app.ws, state, mode, text);
    } else if (s_app.active == XZ_ACTIVE_MQTT_UDP) {
        rt = xz_mqtt_udp_send_listen(&s_app.mqtt_udp, state, mode, text);
    }
    if (rt == OPRT_OK) {
        if (strcmp(state, "start") == 0) {
            xz_on_listen_start_locked(mode);
        } else if (strcmp(state, "stop") == 0) {
            xz_on_listen_stop_locked();
        }
    }
    (void)tal_mutex_unlock(s_app.lock);
...
OPERATE_RET xiaozhi_app_send_abort(const char *reason)
{
    ...
    (void)tal_mutex_lock(s_app.lock);
    if (s_app.active == XZ_ACTIVE_WS) {
        rt = xz_ws_send_abort(&s_app.ws, reason);
    } else if (s_app.active == XZ_ACTIVE_MQTT_UDP) {
        rt = xz_mqtt_udp_send_abort(&s_app.mqtt_udp, reason);
    }
    if (rt == OPRT_OK) {
        xz_on_abort_locked();
    }
